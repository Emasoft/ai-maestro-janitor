...
heartbeat the user globally stopped (and each fire re-reads ~618k cached tokens). It prints the
"globally stopped" guidance instead. With no flag set, the normal arm nudge appears.
...
def test_session_start_skips_rearm_when_globally_disarmed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """kill-switch set → NO /janitor-arm nudge; print the globally-stopped guidance instead.

    The pre-RQ9FIFX6 bug: this hook nudged /janitor-arm unconditionally, so a disarmed machine
    re-armed a fresh ~618k-token-per-fire heartbeat on every new/resumed session.
    """
    out = _run_session_start(monkeypatch, tmp_path, flag="disarm")
    assert "globally stopped" in out, f"expected the stopped guidance, got {out!r}"
...
def test_session_start_nudges_rearm_when_not_stopped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "globally stopped" not in out, "the stopped guidance must not appear when armed"
