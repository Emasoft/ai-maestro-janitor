...
def _wire(
    ...
    monkeypatch.setattr(rotator, "_invoke_slot_capture", lambda email: captured.append(email) or True)
    return captured
...
def test_invoke_slot_capture_launches_uv_run_headful_by_default(
    ...
    assert rotator._invoke_slot_capture("seed@gmail.com") is True
...
def test_invoke_slot_capture_headless_only_when_env_truthy(
    ...
    rotator._bootstrap_pid_path("seed@gmail.com").unlink(missing_ok=True)  # clear the lock
    assert rotator._invoke_slot_capture("seed@gmail.com") is True
...
def test_invoke_slot_capture_is_detached_and_writes_pidfile(
    ...
    pid_path = rotator._bootstrap_pid_path("seed@gmail.com")
...
def test_invoke_slot_capture_skips_if_prior_capture_running(
    ...
    assert rotator._invoke_slot_capture("seed@gmail.com") is False
...
def test_invoke_slot_capture_relaunches_when_prior_pid_dead(
    ...
    rotator._bootstrap_pid_path("seed@gmail.com").write_text("999999", encoding="utf-8")
    assert rotator._invoke_slot_capture("seed@gmail.com") is True
...
def test_cmd_tick_runs_bootstrap_after_cmd_auto(
    ...
    assert rc == 0
    assert order.index("bootstrap") > order.index("auto"), "bootstrap must run AFTER cmd_auto so rotation is never starved"
...
def test_bootstrap_capped_after_max_launches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    captured = _wire(tmp_path, monkeypatch, {"stuck@x.com": {"refresh": None, "session": 20.0, "ba": rotator.MAX_BOOTSTRAP_LAUNCHES}})
    done = rotator._bootstrap_seeded_slots()
    assert captured == []
    assert done == []
...
def test_bootstrap_increments_attempts_on_launch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    captured = _wire(tmp_path, monkeypatch, {"seed@gmail.com": {"refresh": None, "session": 20.0}})
    done = rotator._bootstrap_seeded_slots()
...
def test_bootstrap_resets_attempts_when_recovered(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    captured = _wire(tmp_path, monkeypatch, {"recovered@x.com": {"refresh": "r", "session": 20.0, "ba": 2}})
    done = rotator._bootstrap_seeded_slots()
...
def test_bootstrap_announces_launch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    logs: list[str] = []
    _wire(tmp_path, monkeypatch, {"seed@gmail.com": {"refresh": None, "session": 20.0}})
    monkeypatch.setattr(rotator, "_log", lambda m: logs.append(m))
    done = rotator._bootstrap_seeded_slots()
    assert done == ["seed@gmail.com"]
    assert any("opening a browser" in m and "seed@gmail.com" in m for m in logs)


# ---------------------------------------------------------------------------
# TRDD-56374Z36 — production-log isolation regression + bootstrap browser-launch
# deny-list guard. The bootstrap path calls the REAL `_log`; a rotator test module
# that re-points ROOT but not LOG_FILE leaked "auto-bootstrap: opening a browser …"
# lines into the PRODUCTION rotator.log (re-surfacing TRDD-14IY6MAD / v0.18.2).
# Isolation now comes from the centralized tests/conftest.py autouse fixture; the
# deny-list is the runtime belt so no browser opens for a fixture account.
# ---------------------------------------------------------------------------
def test_bootstrap_run_never_touches_production_log(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """REGRESSION (TRDD-56374Z36): driving the real bootstrap path — which invokes the REAL
    `_log("auto-bootstrap: opening a browser …")` — must write ZERO bytes to the PRODUCTION
    rotator.log. LOG_FILE isolation is provided ONLY by the conftest autouse fixture (NOT
    re-pointed here), so this fails loudly if a rotator test module ever loses that coverage.
    Skips cleanly when the prod root is unresolvable (CI has no such path)."""
    try:
        prod_log = rotator._canonical_rotator_root() / "rotator.log"
    except Exception:  # pragma: no cover - pure Path resolution, never expected to raise
        pytest.skip("cannot resolve the production rotator root")
    before = prod_log.stat().st_size if prod_log.exists() else None

    captured = _wire(tmp_path, monkeypatch, {"seeded@x.com": {"refresh": None, "session": 20.0}})
    done = rotator._bootstrap_seeded_slots()  # calls the REAL _log for the launch announcement
    assert done == ["seeded@x.com"] and captured == ["seeded@x.com"]

    after = prod_log.stat().st_size if prod_log.exists() else None
    assert after == before, "bootstrap wrote to the PRODUCTION rotator.log — isolation leak"
    # And prove the announcement DID land somewhere isolated (the conftest tmp log), so the
    # test cannot pass merely because nothing was logged.
    assert rotator.LOG_FILE != prod_log
    assert rotator.LOG_FILE.exists() and rotator.LOG_FILE.stat().st_size > 0


def test_bootstrap_email_denied_truth_table() -> None:
    """PURE deny-list (TRDD-56374Z36): fixture/placeholder domains are denied case-insensitively;
    every real domain and any unparseable address is fail-open (NOT denied)."""
    denied = rotator._bootstrap_email_denied
    for bad in ("a@x.com", "b@example.com", "c@example.org", "d@test.local", "e@invalid", "f@X.CoM", "g@Example.Org"):
        assert denied(bad) is True, bad
    for ok in ("real@gmail.com", "u@company.io", "v@anthropic.com", "noatsign", "", "@", "z@"):
        assert denied(ok) is False, ok


def test_invoke_slot_capture_refuses_denied_fixture_domain(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Runtime belt (TRDD-56374Z36): the browser-launch path REFUSES a fixture-domain account —
    NO subprocess.Popen, returns False, logs a one-line skip — while a real domain still launches
    (fail-open)."""
    _capture_root(tmp_path, monkeypatch)  # patches ROOT + a subprocess.Popen recorder
    logs: list[str] = []
    monkeypatch.setattr(rotator, "_log", lambda m: logs.append(m))
    assert rotator._invoke_slot_capture("fixture@x.com") is False
    assert _FakePopen.calls == []  # no browser launched for the fixture account
    assert any("refusing" in m.lower() and "fixture@x.com" in m for m in logs)
