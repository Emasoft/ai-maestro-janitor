...
from jevctx.types import Origin, Segment
...
def _span(seg: Segment) -> tuple[int, int]:
    """`Segment.line_span` is `tuple[int, int] | None` in the dataclass (a raw-text
    segment built without one would have none), but `segment()` always sets it -- so
    narrow it here once instead of at every call site (pyright reportOptionalSubscript;
    mypy does not check `tests/`, so this is a pyright-only local patch, see VENDORED.md)."""
    assert seg.line_span is not None
    return seg.line_span
...
def test_line_spans_are_contiguous_and_reconstruct_the_original(name: str) -> None:
    ...
    assert _span(segments[0])[0] == 1
    assert _span(segments[-1])[1] == len(lines)
    for previous, nxt in zip(segments, segments[1:], strict=False):
        assert _span(nxt)[0] == _span(previous)[1] + 1
    for seg in segments:
        start, end = _span(seg)
...
def test_oversized_splittable_segments_are_marked_and_capped() -> None:
    ...
    assert all(s.tokens <= 100 or _span(s)[0] == _span(s)[1] for s in segments)
