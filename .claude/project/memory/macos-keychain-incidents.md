---
name: macos-keychain-incidents
description: "macOS keychain dialog opened hundreds of times / Security wants to use the login keychain with no Always Allow button / cannot type, modal steals focus / keychain prompt flood right after rotating a Claude account / all Claude agents on the machine suddenly report Not logged in / security list-keychains says parameters not valid / does /login fix a dead security session / what is a dangling keychain entry from dotenclave unlock / why does the search list get replaced in my shell rc / why did the flood come back days after I published the fix / what is a staged launchd keepalive closure and why does it revive the old flooder / does pausing the rotator opt-in stop every detector from reading the keychain / keychain-health detector reachability check every heartbeat / how was the 2026-07-09 flood stopped / two independent flooders same night AgentLens / what does macos-keychain govern and who governs it"
ocd: 2026-07-09
lmd: 2026-09-23
metadata:
  node_type: memory
...
The two fleet-down keychain incidents the safe protocol on [[macos-keychain-write-protocol]] and
[[macos-keychain]] was built from: the DEAD SECURITY SESSION (Gotcha 4) and the READ-side
ACL-PROMPT FLOOD (Gotcha 3), plus [[macos-keychain]]'s full edge list (this page carries the
original `## Governed by` / `## Applies to` text, adjacent exactly as authored, because the
flood incident's own footnote citations [^2] and [^8] live inside that edge list's prose).
