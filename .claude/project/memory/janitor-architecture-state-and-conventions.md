...
^ATOM-LMFJ-JEWP [desc: "After an automated clear the SESSION-side summarizer (spawned by SessionStart, has the key) writes the keyed handoff ~10 min later; the daemon never composes (SUMMARY_DELEGATED) and the resume cue wai", keywords: NO_SUMMARY_POST_CLEAR SUMMARY_DELEGATED automated_clear_got_no_summary where_does_the_summary_come_from_after_an_automated_clear session-summary.log summarize_previous_session.py daemon_has_no_OPENROUTER_API_KEY resume_cue_fired_before_the_summary_landed resumed_from_the_stale_handoff summary_hold_active_no_resume agent-handoff_key_file_not_read llm-ext_runs_from_the_session_not_the_daemon two_summarizers_race_on_one_transcript SessionStart_summarizer session-summary.stderr.log, type: project, trdd: TRDD-QZVAEWQH, ocd: 2026-09-02, lmd: 2026-09-02]
The launchd daemon must NEVER summarize, and since 2026-09-02 (TRDD-QZVAEWQH, approach D) it does not: `external_handoff_clear._run` captures the transcript, fires /clear → /janitor-arm → /janitor-resume, prints `SUMMARY_DELEGATED key=<key>` and returns, leaving `summary-pending.json` (the 15-min hold) in place. The SESSION side produces the summary and always had the key: `scripts/summarize_previous_session.py`, spawned detached by `on-session-start.py` on every SessionStart with the session env (the USER's login-shell dotenclave export supplies OPENROUTER_API_KEY to sessions; the daemon Popen()s with no shell; the USER forbids writing the key anywhere on disk). Measured before the change: three of four automated clears already got a keyed handoff from that script ~10 min after the clear (per project .janitor/logs/session-summary.log) while the daemon's compose failed keyless and logged the misleading NO_SUMMARY_POST_CLEAR. The real defect was the RESUME CUE, emitted ~60 s after the clear before the hold check, pointing at the stale handoff SessionStart had injected. Now dispatch checks `summary_hold_active` BEFORE `_phase_clear_resume`, defers the cue while held (≤15 min TTL), and names the fresh `agent-handoff-<key>-*.md` path when it fires. The summarizer's stderr goes to .janitor/logs/session-summary.stderr.log (was DEVNULL). Diagnose a no-summary clear from session-summary.log first. [^16]
...
[^15]: [id: ATOM-HNP8-2ARP, status: valid, supersedes: ATOM-2IUL-97C3, desc: "the cleared AgentlensPro session did NOT resume from precompact-handoff.md — it Read the 22:59 link-only agent-handoff.md written before it was born; five hours of work had no handoff (a regression)", keywords: "resumed_from_precompact-handoff what_did_the_cleared_session_resume_from bootstrap_source_after_automated_clear handoff_predates_the_session cleared_work_has_no_handoff recoverability_regression hold_TTL_only_defers_chores code_comment_intent_vs_observed_injection proxy_read_handoff_source SessionStart_injects_agent-handoff five_hours_of_work_lost_to_a_clear NO_SUMMARY_POST_CLEAR_consequence", ocd: 2026-09-02, lmd: 2026-09-02] DO NOT write that a session cleared without a summary "resumed from the mechanical precompact-handoff.md" — that sentence was the CODE COMMENT's intent for the hold-TTL path, not an observation. BECAUSE on 2026-09-02 the resumed AgentlensPro session (0fc90e0d) never touched precompact-handoff.md: it listed .janitor/state at 04:25:28 and Read the link-only agent-handoff.md (22:59) at 04:25:33 — a handoff written BEFORE the cleared session (f7594ac9, born 23:08) existed, so its five hours / 418k tokens of work were cleared with no handoff covering them; the summary hold only deferred chores (dispatch.log 04:29/34/39). That is a recoverability REGRESSION relative to the pre-79LXF6PJ facts+tail compose, not a degraded mode. DO read the resumed transcript (its SessionStart attachment and first Reads) before naming the bootstrap source, and grade a no-summary fire as "work cleared with no record", never as "resumed from the mechanical handoff". SUPERSEDED BODY: The launchd daemon's children inherit NO harness-injected secrets, just as they inherit no settings.json options (ATOM-0EBD-0XQE): `lib/external_clear.run_llm_ext_summary` spawns `llm-ext` with the daemon's own `os.environ`, and under launchd that carries no `OPENROUTER_API_KEY`. Interactive sessions DO have it — but from the desktop harness that launches Claude Code, not from any file: `~/.zshenv`/`.zprofile`/`.zshrc`/`.zautovenv`, the `.env` candidates, `launchctl getenv`, settings.json's `env` block and the keychain (by that account name) all define nothing, so no rc-sourcing trick can hand it to the daemon. The first live automated clear (AgentlensPro, 2026-09-02 04:23:48, `next-fire-misses`, 418k context) therefore ran its cheap half perfectly (fired 04:23:51, re-armed 04:24:45, resume cue 04:25:03) and its whole point dark: three identical `Remote api 'openrouter-remote' requires 'api_key' (env var $OPENROUTER_API_KEY is not set)` failures, `NO_SUMMARY_POST_CLEAR`, a 15-minute summary hold, and a resume from the stale mechanical `precompact-handoff.md` (a day old). The XCJFCJUX mirror `state.plugin_options_env` covers `CLAUDE_PLUGIN_OPTION_*` ONLY, by design — do not widen it silently; credential placement is a USER ruling (TRDD-QZVAEWQH: settings.json env allowlist / SessionStart capture / llm-ext keychain). Diagnose from `global-state/external-clear.log` — `cold-cache-clear.log` shows only the verdict and the degrade line, never the llm-ext stderr.
[^16]: [id: ATOM-ESLL-4S33, status: valid, desc: "Before designing where a secret should live, check whether the artifact already arrives by another process that has it", keywords: "proposed_putting_the_api_key_in_settings.json daemon_child_has_no_api_key credential_placement_proposal_refused the_summary_was_arriving_from_another_path check_session-summary.log_before_designing NO_SUMMARY_POST_CLEAR_was_misleading resume_cue_did_not_wait_for_the_summary key_must_never_be_written_to_disk user_refused_options_A_B_C one_process_over_already_had_the_key", ocd: 2026-09-02, lmd: 2026-09-02] DO NOT design a credential-placement fix (settings.json allowlist, captured key file, keychain feature) when a launchd child lacks an API key, BECAUSE on 2026-09-02 the working path already existed one process over — the SessionStart-spawned summarizer had the key and was writing the keyed handoff ~10 min after every clear while the daemon logged NO_SUMMARY_POST_CLEAR — and the USER refused every placement outright. DO grep every log that could show the artifact arriving by another route (session-summary.log, the handoff dir by key) before proposing where a secret should live; the defect was the resume cue not waiting, not a missing key.
