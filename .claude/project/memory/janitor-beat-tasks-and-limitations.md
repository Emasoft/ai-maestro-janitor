...
| Tier | Cron | When |
|---|---|---|
| FAST | `*/5 * * * *`  | actively waiting — rate-limit flag, post-compact resume, pending resume directive, `keep-going`, or a pending background agent. **Same as pre-#83**, so recovery latency has ZERO regression exactly when speed matters. |
