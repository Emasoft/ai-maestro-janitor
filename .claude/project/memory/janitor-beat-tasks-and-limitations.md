...
^ATOM-56QK-O652 [desc: "arm_prepare.py staggers the heartbeat cron to {offset}-59/N (offset=sha256(project_dir)%N) and every hook timeout widened to 10s; every cron reader must now go through cron_period.py, never startswith", keywords: UserPromptSubmit_hook_timed_out_after_2s why_did_my_hook_time_out_under_load heartbeat_cron_stagger_per_project_offset offset-59/N_cron_shape sha256_project_dir_modulo_N hook_timeout_widened_to_10_seconds CPU_run-queue_wait_load_average_50-117 all_sessions_heartbeat_fire_in_the_same_minute cron_period.py_period_minutes_parse do_not_parse_cron_with_startswith_slash-star is_the_external_clear_next-fire_fix_landed_yet, ocd: 2026-09-24, lmd: 2026-09-24]

Commit 7ed4cdeb (2026-09-24; measurement report: LOCAL scope). Under load, every armed session shared the identical */N * * * * cron grid, so a host's heartbeat prompts -- and every hook each one fires -- all started in the same wall-clock minute; measured burst: 30-60 concurrent hook launches pushed 1.4-2.0s of CPU run-queue wait at load average 50-117, which then tripped hooks armed with a 2-3s timeout sized for an UNcontended launch (the hook's own work is ~0.1s: ~45ms Python startup + ~45-50ms of imports). Two coupled fixes landed together: (1) arm_prepare.py now STAGGERS the minute-step cron it arms onto a per-project offset: */N * * * * -> {offset}-59/N * * * *, offset = sha256(project_dir) % N, stable across re-arms of the same project -- so a host's sessions no longer all fire in the same minute. (2) every hook's own timeout widened from 2-3s to 10s (hooks/hooks.json) so a launch still landing in a smaller residual cluster has headroom. A staggered cron is NOT the plain */N shape any more: every reader of the armed/desired cron string (fleet_scan.stale_threshold_for, orphaned_resume.cadence_seconds, and any future one) MUST go through the new shared parser scripts/lib/cron_period.py::period_minutes() (or its parse() -> (offset, step)), never a hand-rolled check -- a reader using field.startswith('*/') silently falls back to its own stricter default staleness window on the new {offset}-59/N form instead of the real period. As of 2026-09-24, this landed for every remaining reader too: commit 9efd5df3 gave scripts/lib/external_clear.py::seconds_until_next_fire the same treatment (it now delegates to cron_period.parse() as well), closing the one reader 7ed4cdeb's own follow-up had left open. [^7] [^8]
...
^ATOM-XRW5-KCG6 [desc:"a heartbeat fire costs context x NUMBER OF TOOL CALLS, not context x 1 — measured at 3.2M tokens and $1.72 for one six-call fire", keywords: how_much_does_one_heartbeat_fire_actually_cost why_is_the_janitor_the_top_token_consumer heartbeat_cost_per_fire_dollars cache_read_millions_of_tokens_on_an_idle_session is_a_quiet_fire_cheap does_cost_scale_with_context_or_with_api_calls why_is_a_janitor-quiet_reply_the_biggest_cost_lever how_do_I_measure_heartbeat_cost_with_agentlenspro why_was_my_cost_estimate_wrong_by_18x how_many_api_calls_did_the_measured_fire_make, ocd: 2026-08-04, lmd: 2026-08-04]

MEASURED, not estimated (`agentlenspro heartbeat-cost`, 2026-08-04): ONE heartbeat fire on a 406k-token session cost **3,239,112 tokens = $1.7153** — cache_read 3,231,555, output 2,792, across **6 API calls** in 15.0s. The law this establishes, and the one I had wrong: a fire costs `context x NUMBER OF API CALLS IN THE TURN`, not `context x 1`. Every tool round-trip re-reads the whole transcript, so a fire that makes 6 tool calls costs SIX times a fire that answers immediately. My own estimate before measuring was ~18k weighted per fire; the true figure is ~323k weighted — wrong by 18x, and wrong in the direction that makes idle sessions look harmless. CONSEQUENCES: (1) the zero-output contract is not politeness, it is the single biggest cost lever — a `[janitor-quiet]` fire answered with the EMPTY string is ~1 call, an investigated one is 6+; (2) the agent's own curiosity, not the cron cadence, is usually why a janitor session tops the fleet burn table — halving the cadence saves less than not investigating on a quiet fire; (3) never estimate this from context size alone, ALWAYS measure with `agentlenspro heartbeat-cost`. See [[janitor-tool-call-cost-law]] and [[window-burn-rate-alarm-contract]]. [^3]
...
[^7]: [id: ATOM-E2J6-6R1E, status: valid, desc: "Guardrail: any new reader of the heartbeat cron string must go through cron_period.py, never a hand-rolled startswith('*/') check.", keywords: "never_parse_cron_with_startswith_slash-star cron_reader_silently_falls_back_to_wrong_default staggered_cron_offset-59/N_shape does_my_code_understand_the_staggered_cron cron_period_period_minutes_parse_helper fleet_scan_stale_threshold_for_wrong_default orphaned_resume_cadence_seconds_wrong_default my_staleness_detector_uses_the_wrong_window why_is_my_cadence_check_off_after_the_stagger_fix writing_a_new_reader_of_the_armed_cron_string", ocd: 2026-09-24, lmd: 2026-09-24] DO NOT parse an armed/desired heartbeat cron string with field.startswith("*/"), BECAUSE the janitor-armed cron is now the staggered {offset}-59/N * * * * shape (7ed4cdeb, arm_prepare._stagger), not the old plain */N -- a startswith("*/") check silently misses it and falls back to a stricter default staleness window instead of the real period, which is exactly what orphaned_resume.cadence_seconds and fleet_scan.stale_threshold_for used to do. DO delegate to scripts/lib/cron_period.py::period_minutes() (or parse() for the raw (offset, step) tuple) instead of hand-rolling a check.
[^8]: [id: ATOM-G9A2-UNI5, status: valid, supersedes: ATOM-56QK-O652, desc: "Correction: the external_clear next-fire follow-up landed in 9efd5df3 on 2026-09-24; it is no longer uncommitted/in-progress.", keywords: "is_the_external_clear_fix_landed seconds_until_next_fire_staggered_cron cron_period_parse_delegation 9efd5df3_landed_commit do_not_call_this_uncommitted_any_more external_clear_cron_period_integration stale_in-progress_status_corrected TRDD-D7RLXAN1_follow-up_landed verify_before_repeating_an_in-progress_claim external_handoff_clear_headroom_check", ocd: 2026-09-24, lmd: 2026-09-24] DO NOT describe the external_clear.py seconds_until_next_fire staggered-cron fix as an uncommitted or in-progress follow-up, BECAUSE it LANDED in commit 9efd5df3 (2026-09-24): seconds_until_next_fire now delegates to cron_period.parse(), the same shared parser period_minutes() already used. DO cite 9efd5df3 as landed, and check git log before repeating a stale in-progress claim. SUPERSEDED BODY: Commit 7ed4cdeb (2026-09-24), reports/hook-timeout/20260924_175736+0200-startup-timing.md. Under load, every armed session shared the identical */N * * * * cron grid, so a host's heartbeat prompts -- and every hook each one fires -- all started in the same wall-clock minute; measured burst: 30-60 concurrent hook launches pushed 1.4-2.0s of CPU run-queue wait at load average 50-117, which then tripped hooks armed with a 2-3s timeout sized for an UNcontended launch (the hook's own work is ~0.1s: ~45ms Python startup + ~45-50ms of imports). Two coupled fixes landed together: (1) arm_prepare.py now STAGGERS the minute-step cron it arms onto a per-project offset: */N * * * * -> {offset}-59/N * * * *, offset = sha256(project_dir) % N, stable across re-arms of the same project -- so a host's sessions no longer all fire in the same minute. (2) every hook's own timeout widened from 2-3s to 10s (hooks/hooks.json) so a launch still landing in a smaller residual cluster has headroom. A staggered cron is NOT the plain */N shape any more: every reader of the armed/desired cron string (fleet_scan.stale_threshold_for, orphaned_resume.cadence_seconds, and any future one) MUST go through the new shared parser scripts/lib/cron_period.py::period_minutes() (or its parse() -> (offset, step)), never a hand-rolled check -- a reader using field.startswith('*/') silently falls back to its own stricter default staleness window on the new {offset}-59/N form instead of the real period. Known follow-up, UNCOMMITTED as of 2026-09-24 (working tree only, not landed on main): scripts/lib/external_clear.py::seconds_until_next_fire has a third, pre-existing copy of the same */N-only pattern (needs the fire offset as well as the period) -- do not describe this follow-up as landed.
