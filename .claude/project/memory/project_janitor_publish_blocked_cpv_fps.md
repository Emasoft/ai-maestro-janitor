...
prose instead. The `.cpv-audit-consent.json` registry was considered and rejected per 65078efa. [^7] [^8] [^9]
...
^ATOM-LE4O-6B4F [desc: "CPV blocks any os.environ[dynamic_key] = value from file content (ENV_INJECTION, MAJOR) and its consent sentinel does NOT cover that rule — read options through a dict mirror + accessor, never write t", keywords: publish_blocked_ENV_INJECTION skillaudit_persistence_ENV_INJECTION os.environ_dynamic_key_write_flagged CPV_MAJOR_environment_variable_injection audit_consent_sentinel_not_working sentinel_did_not_demote_finding WARNING_the_following_code_could_be_malicious _EXECUTION_CLASS_RULES settings.json_env_block_into_os.environ plugin_options_from_settings_file mirror_dict_instead_of_os.environ plugin_option_accessor, type: project, trdd: TRDD-XCJFCJUX, ocd: 2026-09-02, lmd: 2026-09-02]

Publish #5 (3.4.4, 2026-09-02) failed at the remote CPV gate with ONE MAJOR: `skillaudit:persistence ENV_INJECTION` on `os.environ[key] = text` in a loader that mirrored `~/.claude/settings.json`'s `CLAUDE_PLUGIN_OPTION_*` into the process environment. CPV's three Python carve-outs (literal key+value, build/cache-dir vars, read-modify-write with a nearby env read) correctly do not match a dynamic key with a file-sourced value, and the documented audit-consent sentinel comment does NOT demote it: `_EXECUTION_CLASS_RULES` omits `ENV_INJECTION` in every CPV version (5.14.2 cache and the gate's pinned checkout) — filed as claude-plugins-validation#223. A launchd-plist `EnvironmentVariables` route was rejected too (the installer's plist is a scanned heredoc by design, and a test forbids env in it). What passed: no environment writes at all — `state._SETTINGS_OPTIONS` dict mirror filled from the file (re-read on mtime), ONE accessor `state.plugin_option(name)` (a real env var always wins, presence not truthiness), `is_truthy_env`/`_env_interval`/daemon-lane reads routed through it, and `state.plugin_options_env()` merged into the child env at the daemon's spawn points. Same shape the file already used for CLAUDE_PROJECT_DIR (module-local override, env wins). Do NOT retry the sentinel or a plist env block for this class; do NOT search for a regex-evading spelling — the accessor is the design.
