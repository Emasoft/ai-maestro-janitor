...
When a publish is interrupted AFTER `[G1] version bump` created the local tag but BEFORE the push, the next run prints `Local plugin.json is at <N> but origin is at <N-1> — using remote as bump baseline (interrupted-publish recovery)` and re-bumps to the SAME version, producing a second `chore: bump version to <N>` commit — but it does NOT re-point the pre-existing local `v<N>` tag, so the tag it pushes names the FIRST bump commit. Measured 2026-09-04: `v3.4.14` landed on `cc6bc25a` while `origin/main` was `4326519d`, four commits ahead. Benign that day (the four were two version bumps plus two test/comment-only fixes, and the shipped code was identical), but a functional commit in that window would ship a release missing it. ALWAYS verify after a recovered publish: `git rev-parse v<N>^{commit}` vs `git rev-parse origin/main`, and `git log --oneline v<N>..origin/main`. [^17]
