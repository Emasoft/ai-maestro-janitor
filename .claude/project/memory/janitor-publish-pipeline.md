...
TELL THEM APART BY THE FAILURE TEXT, not by which file failed: empty stdout ⇒ A or B; a raw `TimeoutExpired` ⇒ D; an `elapsed < N` assertion ⇒ C; a literal `'timeout'` where a different classification was expected ⇒ B. [^11] [^12]
