...
^ATOM-APUY-M2DB [desc: "Settings traps from the 2.1.241-247 window: CLAUDE_CODE_PROJECT_DIR_NAME would desync every LOCAL memory path from the harness, and promptCacheTtl made the cache split configurable", keywords: CLAUDE_CODE_PROJECT_DIR_NAME project_slug_returns_the_wrong_dir the_LOCAL_memory_dir_points_at_the_wrong_path per_project_transcript_dir_override fleet_scan_resolves_every_project_to_the_same_slug why_is_every_project_sharing_one_memory_dir promptCacheTtl_and_subagentPromptCacheTtl cache_TTL_now_configurable_not_a_harness_constant Sonnet_5_auto_compact_at_967k_not_934k modelPricing_managed_setting hourly_vs_5min_prompt_cache_split settings_that_bite_before_you_notice_them, ocd: 2026-09-26, lmd: 2026-09-26]

Two settings-shaped items worth knowing before they bite: **`CLAUDE_CODE_PROJECT_DIR_NAME`** (2.1.234) lets a host override the per-project transcript dir with a short custom name, while `memory_scopes.project_slug()` still derives that directory purely by dashing every non-alphanumeric in the absolute path — if a host ever sets it, LOCAL memory, LOCAL TRDDs and several detectors silently address a directory the harness is not using. And **`promptCacheTtl` / `subagentPromptCacheTtl`** (2.1.243) make the 1-hour-main / 5-minute-subagent split a CONFIGURABLE value, not the harness constant several janitor comments still describe it as.
...
^ATOM-L1OG-ZN6M [desc: "CC 2.1.257 fixed 3 upstream prompt-cache-miss sources — advisor-model sessions missed on every background request, Remote Control re-sent the Bash tool def, oversized screenshots missed each turn", keywords: prompt_cache_miss cache_miss_every_background_request advisor_model_cache_miss fable_advisor_quota_burn compaction_uncached recap_uncached prompt_suggestions_uncached remote_control_bash_tool_definition_resent screenshot_cache_miss image_size_cap_cache_miss claude_code_2.1.257 CLAUDE_CODE_SUBAGENT_MODEL_FORCE effort_session_only detached_setsid_background_command_reaped defaultMode_bypassPermissions_ignored, type: project, ocd: 2026-09-01, lmd: 2026-09-01]

Claude Code 2.1.257 (2026-09-01) fixed three prompt-cache-miss sources UPSTREAM, none of them janitor-fixable: (a) a session with an ADVISOR model set (the Fable advisor) missed the prompt cache on EVERY background request — compaction, /recap, prompt suggestions — and re-sent the whole conversation uncached; (b) Remote Control connecting mid-session re-sent the Bash tool definition (one miss per connect); (c) screenshot-heavy sessions missed on every turn once past the image size cap. Consequence for diagnosis: a quota burn measured BEFORE 2.1.257 on an advisor-running host (this repo's TRDD-2F3I2P18 incident) plausibly had (a) as a large share — do not attribute the whole burn to the janitor's own cadence, and do not expect the same burn rate after the upgrade. Also new in 2.1.257: `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (overrides per-spawn and agent-definition models); `/effort s` session-only effort (still a prefix rewrite — `external_clear.prefix_invalidated` counts it because it diffs the per-turn effort column); detached (setsid) background commands no longer survive a background-task stop or CC exit — MEASURED 2026-09-02: a foreground Bash call's setsid child still survives the call ending, reparented to pid 1, so the heartbeat's daemon spawn is not reaped by the spawn itself; project-scope `defaultMode: bypassPermissions` is ignored (user/managed only). Source: TRDD-NUD3DGX5.
