...
- **Findings pipeline (v0.51.0 — TRDD-FENWWB4E + TRDD-4649ZLE0, ARCHITECTURE.md §4/§5):**
  ...
  fleet github-config digest. [^1] [^2]
...
[^2]: [id: ATOM-QP4H-ZY1H, status: valid, desc: "measured 2026-08-26: active-alerts.json had a 6-minute mtime while the message inside was frozen 3.6h earlier — I quoted its numbers as live and relayed a remedy that was a no-op", keywords: "the_alert_file_was_updated_minutes_ago_so_I_trusted_its_numbers mtime_is_fresh_but_the_message_inside_is_stale I_quoted_a_stored_alert_message_as_a_current_measurement active-alerts_json_rewritten_without_recomputing_the_text how_old_is_the_claim_versus_how_old_is_the_file I_relayed_a_remedy_that_did_not_apply_on_this_host", ocd: 2026-08-26, lmd: 2026-08-26] DO NOT infer a stored alert's CONTENT age from the FILE's mtime, BECAUSE delivery bookkeeping rewrites the file without recomputing the message: `active-alerts.json` read 0.1h old while its `message` string was frozen at `firstSeenAt`, 3.6h earlier. I quoted its "Only the Fable window is spent (100%)" to the owner and to a peer as a current measurement; no usage probe carries a Fable window at all (all `seven_day_fable` null, the only model window `nimbus_quill` at 0%), and the peer independently measured that the remedy it named — "move agents OFF Fable" — is a no-op here, 0 of 13 registered agents being Fable-backed. DO read the embedded `firstSeenAt` / generation timestamp, not `os.path.getmtime`, and re-derive any NUMBER before repeating it — a stored alert is a SNAPSHOT with its own clock, and the same file can be simultaneously fresh and wrong. This is the sibling of the append-only ledger trap on this page: there the entry never expires, here the entry is rewritten but its payload does not. Both fail the same way — a reader who checks recency instead of the underlying artifact reports a resolved or stale condition as live.
