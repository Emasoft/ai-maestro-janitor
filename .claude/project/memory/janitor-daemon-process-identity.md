...
^ATOM-36U3-AA2V [desc: "installed is not running — after a plugin update the daemon self-respawns onto the newer STAGED tree, and the proof is a daemon.log respawn line plus the new pid's start time, not the plugin cache", keywords: I_updated_the_plugin_but_is_the_daemon_running_the_new_code claude_plugin_update_says_restart_to_apply_changes does_the_daemon_pick_up_a_new_version_by_itself newer_version_staged_exit_for_respawn is_the_fix_live_or_just_installed the_daemon_pid_changed_after_the_update grep_the_plugin_cache_or_the_staged_tree which_tree_does_the_running_daemon_execute how_do_I_prove_the_daemon_reloaded daemon_still_running_the_old_version_after_publish, type: reference, ocd: 2026-09-03, lmd: 2026-09-03]

`claude plugin update` prints `Restart to apply changes.` — true of the SESSION, misleading about the DAEMON, which is a separate long-lived process. The daemon's os-keepalive detects a newer staged version on its own and restarts itself: `daemon.log` shows `os-keepalive: newer version staged → exit for respawn` then `stopping (self-update-respawn)` then `started (pid=<new>, tasks=[...])`. Measured 2026-09-04: 3.4.14 installed ~00:54, daemon pid 40810 exited and pid 60621 started 00:57:45 — under four minutes, no human action. To PROVE the running daemon carries a fix, grep the STAGED tree under the plugin DATA dir (`<DATA>/scripts/...`), NOT the plugin cache the session loads — they are different trees, and the daemon runs the staged import closure. Cross-check the new pid's `ps -eo pid,lstart` start time against the install time. [^5]
...
[^5]: [id: ATOM-OGBL-P76R, status: valid, keywords: "I_shipped_the_fix_so_the_behaviour_is_fixed installed_is_not_the_same_as_running restart_to_apply_changes verified_the_plugin_cache_not_the_staged_tree is_the_fix_live_or_just_installed the_daemon_pid_changed_after_the_update how_do_I_prove_the_daemon_reloaded grep_the_plugin_cache_or_the_staged_tree which_tree_does_the_running_daemon_execute daemon_still_running_the_old_version_after_publish", ocd: 2026-09-03, lmd: 2026-09-03] DO NOT report a daemon-side fix as live because the publish went green and the plugin installed, BECAUSE the session and the daemon load from DIFFERENT trees and the daemon's respawn is asynchronous — for a few minutes the installed fix is genuinely inert. DO close the loop with three cheap facts before saying it works: the respawn line in `daemon.log`, the new pid's `lstart` after the install time, and the fix's own text grepped out of the STAGED tree. Code identity is what these prove; behaviour is proven by the test suite; live behaviour remains unobserved until an actual episode — say which of the three you have.
