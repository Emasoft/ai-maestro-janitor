...
NEXT ACTION: discriminate the three candidates BEFORE writing any fix — (1) read the stub's fallback branches (does it print [janitor-quiet] itself when dispatch cannot be resolved or launched mid cache-swap?); (2) read main()'s phase order around the fire-log write and the keep-going phase (does keep-going log on every fire after the write, or is its line deduped?); only then decide whether the stderr line belongs in the stub or in dispatch. The body's fix and test sentences presuppose candidate (A) and are superseded by the Approval log: stderr only, and a test that pre-creates heartbeat-fires.log as a directory. Nothing in this card is established beyond the three facts: quiet token printed, no fire line, no dispatch.log line, on the first fire after the 3.5.5 cache swap at 12:47 local.
