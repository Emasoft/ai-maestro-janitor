...
implementation-commits: [0c607b36, b651192a, 8223e234]
---
...
- 2026-09-24 13:37 — column todo, code landed: 0c607b36 (reservation-first injected selection, shared with TRDD-U6C3YXEL), b651192a (F1: a task-notification shows its verbatim <summary>/<result> excerpt plus a pointer, never its id/path wrapper; the whole-tool 20-char gate counts only the result after the call-echo line) and 8223e234 (F1b: an inline notification is gated on the excerpt it SHOWS; the label follows what survived the cut). --out byte-identical on the 4 cached sessions after each.
- Owner-message side effect of b651192a is disclosed under Review corrections (4eb7bf5d: three older owner messages became pointers; b2bf5b7b: one swap), and so is 8223e234's (fd5cc3e0: the freed bytes went to owner messages, inline owner 7 -> 12, an undecided fill-order consequence; b2bf5b7b: "i had to manually rotate again. why?" inline only as a byte-budget side effect, the U6C3YXEL design limit is not fixed).
...
Not closable on 0c607b36: read by hand, 2 of b2bf5b7b's 3 non-owner items are a message intro and task-notification metadata, and fd5cc3e0's 3 of 3 are metadata shells; non-owner items inline went 4/4/8/4 -> 3/4/4/3. The whole-tool 20-character bar also passes a bare call echo, since the body starts with the synthetic name(input) line. F1 (event content gate) landed as b651192a on 2026-09-24 and addresses both; its verification found one remaining defect, recorded below (F1b).
