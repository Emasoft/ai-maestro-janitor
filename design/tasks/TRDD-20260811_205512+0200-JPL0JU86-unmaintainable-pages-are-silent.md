...
## ⏵ 2026-08-26 21:26 — a SECOND population the same mechanism would serve, with a measured cost

**Stated first so it is not conflated: this is NOT this card's defect.** This card's Why is
specific — four USER-scope pages that are SYMLINKS resolving outside their own root. What follows
is a different population that happens to need the same *mechanism* (a durable "permanently not a
candidate" marker), and it arrived with a price tag.

A `[janitor-memory-atomize]` marker fired tonight. The agent claimed USER scope, was handed **5
candidates — all of them `debugging-methodology-*` navigation/map stubs** generated by earlier
splits — read each in full, and declined all 5 as `free-prose-leaf-no-distinct-facts`: pure map
pages whose facts and lessons all live on linked sub-pages. That verdict is CORRECT and the pass
did the right thing; it recorded refusals so they self-expire in 7 days or on content change.

**The cost of that correct null result: 208,696 subagent tokens, 13 tool calls.** And the refusal
expires, so unless those pages change — and a split-generated map page is precisely the kind that
does not — the same 5 re-nominate in 7 days and the same ~200k is spent to reach the same answer.

**Why it belongs on this card and not a new one:** the fix this card asks for is a page-level
assertion that no chore can ever act here, replacing a silent per-run abstention. A 7-day
expiring refusal is the *weaker* form of exactly that — it re-asks a question whose answer is
structural. If the mechanism lands, the natural question is whether its predicate covers
"free-prose leaf with zero durable facts" as well as "symlink outside the root". **That is a
scoping decision for whoever takes this card, not something to assume** — the two populations are
unmaintainable for different reasons, and a marker that conflates them could silence a page that
merely has nothing to do *today*.

Recorded as evidence for the A/B decision this card is parked on, not as a change to it.
