...
updated: 2026-09-16T22:39:14+0200
...
> Read from the traceback maps (`reports/board-drain/20260905_203100+0200-{nauto,n4}-failure-map.txt`):
> only `external_clear_retry::test_a_real_failing_binary_is_classified_not_swallowed`
> raises `TimeoutExpired` (`fake-llm-ext.sh … 100.0 seconds`, both runs); the other 12 carry
> no raised `TimeoutExpired` (a caught-and-silenced timeout would leave no trace, so this is
> "no category-D raise", not "not timeout-caused"). A read-only worker is quoting the 12
> failure lines now (no pytest); result lands as the next entry. A quiet start did not
> clear them, and the 13:24 entry above says they pass in isolation — so the mechanism is
> still "inside the full suite", now with a stable 13-id core to instrument.
> `test_branch_protection_guard.py` fails a different sub-test in each run (3+3).
>
...
- 2026-09-16T22:39:13+0200 — for the next reader: the test ids this card's own '## The failures' code block still owned when cancelled (rows 1 & 2, test_capture_all_logins.py, moved to TRDD-K7WQ2NRB per the card's SPLIT note; rows 3-11 remain, 9 ids not ten — count discrepancy flagged, not resolved): tests/test_marketplace_refresh_scoped.py::test_disabled_plugins_are_skipped, tests/test_marketplace_refresh_scoped.py::test_per_session_refreshes_each_unique_marketplace, tests/test_gh_reply_watch.py::test_the_inbox_does_not_leak_another_projects_threads, tests/test_gh_reply_watch.py::test_a_stale_inbox_falls_back_to_todays_exact_behaviour, tests/test_gh_reply_watch.py::test_the_floor_expires_so_a_later_fire_polls_again, tests/test_memory_librarian.py::TestMemoryLibrarianReindex::test_reindex_invoked_for_the_local_root, tests/test_memory_librarian.py::TestMemoryLibrarianReindex::test_reindex_runs_before_index_query, tests/test_memory_librarian.py::TestMemoryLibrarianReindex::test_reindex_failure_is_tolerated, tests/test_token_usage_anomaly_detector.py::test_alarm_enriched_with_agentlens. NOTE: the card's later STATE entries (11:40 rerun) say the marketplace_refresh_scoped/gh_reply_watch/memory_librarian trio PASSED after the 5b5267a5 fixes and 9 NEW branch_protection* failures appeared instead — those branch_protection ids are NOT individually enumerated anywhere in this card's text, only as family names. If any of the ids above, or the branch_protection family, goes red in CI or the publish gate, open a NEW card citing TRDD-Q8PNPRTW — this card's STATE holds the serial-isolation results and evidence pointers (reports/suite-failures/raw-captures-20260904/, reports/suite-soak/, reports/suite-failures/20260916_094059+0200-*). An owner veto of this cancellation is the same act: a new card.
