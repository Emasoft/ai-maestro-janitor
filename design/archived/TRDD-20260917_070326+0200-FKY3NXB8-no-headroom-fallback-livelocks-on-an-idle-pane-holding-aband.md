...
[2026-09-17T14:56:36+0200] CLOSED: fix 9803ee14 landed with its Expect-gated clear step and the human-draft guard test; three acceptance boxes ticked from the STATE record; gate green on d09fe94b. Moving to complete.

## Acceptance criteria

- [x] pane_policy._at_idle's NO_HEADROOM row clears the janitor's own leftover '/model' text with one ESC verified by Expect.FIELD_EMPTY (the wedge row's mechanism, not a blind key) and then performs the model switch -- landed in 9803ee14.
- [x] A human draft that merely starts with the letters '/model' is NOT cleared: only the whole command word (text == '/model' or startswith('/model ')) matches -- test_idle_no_headroom_does_not_clear_a_human_draft_that_merely_starts_with_model.
- [x] Any other idle text still defers with a logged reason (first 40 chars); pane tests 140 passed and the full gate is green on d09fe94b (ruff/mypy/pyright clean, pytest 16793 passed / 0 failed).
