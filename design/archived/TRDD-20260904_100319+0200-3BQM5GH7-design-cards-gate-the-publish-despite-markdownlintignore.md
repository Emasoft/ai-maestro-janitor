...
- 2026-09-05T10:38:00+0200 — COMPLETED by main-session under the USER's standing autonomous-drain permission (memory ATOM-CCRI-ZRT2, 2026-09-03; re-issued as today's session goal). RULING: option 3 — no repo-side lint gating change; design cards keep satisfying CPV's `--strict` gate, because the card's own strongest finding is that every past MD056 hit was a REAL defect. Also applied: `.markdownlintignore` DELETED (`git rm`) — grepped 2026-09-05 in exactly three places — scripts/, .github/, and the installed CPV scripts — and none reads it. NOT grepped: `.pre-commit-config.yaml` (markdownlint-cli v1 hooks DO honour the file), the pre-push hook body, mega-linter config, editor plugins. The card itself scoped its claim to 'no tool MEASURED in this pipeline'; the deletion is recoverable by `git revert` if an unmeasured reader turns up. The CPV doc/UX courtesy issue is filed separately.
