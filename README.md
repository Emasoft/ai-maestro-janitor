...
[![version](https://img.shields.io/badge/version-3.6.1-blue)](https://github.com/Emasoft/ai-maestro-janitor/releases/latest)
...
(plugin updates, OAuth keepalive) are coordinated so the two daemons
...
| `task-pr-mismatch` | 30 min | Session tasks whose status contradicts the state of a referenced PR. Needs the opt-in task tools (see below). |
| `stale-task` | 30 min | Tasks stuck `in_progress` >2h or `pending` >24h with no update. Nudges to resume, close, or defer. Needs the opt-in task tools: Claude Code 2.1.233 removed `TaskCreate`/`TaskUpdate`/`TaskList`/`TodoWrite` on Opus 4.8, Sonnet 5, Fable 5, Mythos 5 and newer, so on a default modern session nothing writes `~/.claude/tasks/` and this detector is correctly silent. `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` brings both back. |
...
| ~~`user-plugins-update`~~ | — | **RETIRED 2026-08-20 (TRDD-E39YT9G6).** The Claude Code harness self-updates user-scope plugins from its `autoUpdate:true` refreshed catalogs, so the daemon's hourly sweep (and the per-session watchdog shim that lived in this row) duplicated harness work — and under host load the sweep's serial spawns each timed out until the workload cap killed the child. History: pre-daemon this was "Track 1" running a sweep PER SESSION ([issue #7](https://github.com/Emasoft/ai-maestro-janitor/issues/7)); v0.5.2 moved the sweep into the daemon; TRDD-TIZHEPNC un-absorbed it from the server (which deleted its own copy for the same reason); E39YT9G6 retired it everywhere. The TARGETED per-plugin request consumer in the daemon remains. |
...
| `oauth-login-needed` | 6 h | **Opt-in (OAuth account rotator), macOS.** Silent unless a rotator home exists. Surfaces the accounts that need a ONE-TIME human login — the ones that can neither self-renew (no refresh token) nor auto-bootstrap (no live claude.ai session) — naming the resolved `open-login.sh <email>` for THIS host (opens a DEDICATED Chrome window; your default browser stays untouched). A secondary `[oauth-capture-stalled]` line covers logged-in-but-not-yet-captured accounts. A periodic `[OAUTH-LOGIN-TOPUP]` nudge (`/janitor-capture-all-logins`, every `CLAUDE_PLUGIN_OPTION_LOGIN_TOPUP_EVERY_DAYS` days, default 7) proactively tops up every account before any of them expire. After you log in once, the daemon auto-bootstraps a refresh-bearing slot and the account self-renews thereafter (TRDD-32acd15f). |
...
- `/janitor-branch-protection-setup` — one-shot interactive setup of the
  ...
  admin on the repo, or no `repository` URL is declared. The silent
  auto path (no human in the loop) is **ON by default** since 2026-08-12:
  `scripts/guard/branch_protection_apply.py` applies the same ratified
  baseline on the heartbeat, so an unprotected default branch is fixed
  rather than merely flagged. It still skips any project that is not a
  plugin with a resolvable `repository` slug, and `/janitor-autofix-off`
  or `guard_mode_enabled = false` opts a project out.
...
| `daemon_enabled` | true | When true, per-session heartbeats lazy-spawn the global janitor daemon (`scripts/daemon.py`) on the `<global-state>` dir (canonically `${CLAUDE_PLUGIN_DATA}/global-state/` since TRDD-2U8AH82F), which owns every machine-global auto-update task (janitor self-update, user-scope plugin updates). Singleton via exclusive flock — N sessions = ONE daemon. Manual kill switch: `touch <global-state>/kill-switch.flag` (running daemon exits on next loop tick). |
