...
| `marketplace-refresh` | 5 min | **v0.5.2+: per-session shim.** Calls `ensure_daemon_running()` then exits silently. The actual `claude plugin marketplace update` (bulk refresh of every configured marketplace globally) is now owned by the single global janitor daemon (`scripts/daemon.py`, lazy-spawned by any heartbeat, flock-singleton at `~/.claude/janitor-global-state/daemon.flock`). This fixes [issue #7](https://github.com/Emasoft/ai-maestro-janitor/issues/7): pre-daemon, N concurrent Claude Code sessions = N concurrent marketplace-refresh workers because each per-project PID gate looked at its own state dir and saw "no prior worker." The daemon's flock ensures exactly one worker system-wide, regardless of how many sessions are armed. Surfaces a stale-daemon drift line at most once per UTC hour when the daemon's `marketplace-refresh.last-run.ts` is older than 2× cadence. |
...
auto-update task — `marketplace-refresh` and `user-plugins-update`. It is
...
| `~/.claude/janitor-global-state/marketplace-refresh.last-run.ts` | last successful task completion stamp |
...
| `daemon_marketplace_refresh_interval` | 1800 | Min seconds the daemon waits between `claude plugin marketplace update` runs. 30 min by default — aggressive cadences are unnecessary because the daemon is the only writer system-wide. |
