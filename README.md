...
| `marketplace-refresh` | 5 min | Calls `claude plugin marketplace update` (no arguments) DETACHED in the background. The bare bulk form is the only Claude Code primitive that handles cross-marketplace metadata refresh in a single call — it covers EVERY configured marketplace, including those for plugins installed at `user` or `managed` scope (which `plugin-updates` won't touch) and those whose plugins are not yet installed locally. Tracks the detached PID in `.janitor/state/marketplace-refresh.pid`; subsequent fires that find the prior child still alive log a "still running, skipping" line and exit immediately, so successive heartbeats never overlap on the same git indices. Silent on stdout — pure background maintenance; every detach/skip is logged to `.janitor/logs/marketplace-refresh.log` for auditability. |
