...
| `mcp-config-drift` | 1 h | Audits the project's MCP server configuration. Checks: (a) `<project>/.mcp.json` parses as valid JSON, (b) `.mcp.json` is either git-tracked OR gitignored — never both unset (the ambiguous case is the #1 source of "I added the server but my teammate's checkout doesn't have it" or accidentally committing a personal token), (c) every server in `.mcp.json` and the `.mcpServers` blocks of `.claude/settings.json` / `.claude/settings.local.json` declares either a `command` (stdio) or `url` (http/sse/ws) transport, (d) every `$VAR` / `${VAR}` reference in commands, args, env values, headers, or URLs is set in the current shell. Same project-scope-only mandate as `plugin-updates` — never inspects `~/.claude/settings.json`. |
