...
| `marketplace-refresh` | 5 min | Spawns `claude plugin marketplace update` (no arguments) DETACHED in the background. The bare bulk form is the only Claude Code primitive that handles cross-marketplace metadata refresh in a single call — it covers EVERY configured marketplace, including those for plugins installed at `user` or `managed` scope (which `plugin-updates` won't touch) and those whose plugins are not yet installed locally. Tracks the spawned PID in `.janitor/state/marketplace-refresh.pid`; subsequent fires that find the prior child still alive log a "still running, skipping" line and exit immediately, so successive heartbeats never pile up concurrent fetches on the same git indices. Silent on stdout — pure background maintenance; every spawn/skip is logged to `.janitor/logs/marketplace-refresh.log` for auditability. |
| `user-plugins-update` | 5 min | **Track 1 of the auto-update directive — cron-global, project-agnostic.** Spawns a detached worker that iterates every `user`-scope plugin (enabled and disabled) via `claude plugin list --json` and runs `claude plugin update <id> --scope user` per plugin sequentially. The worker PID is tracked in `.janitor/state/user-plugins-update.pid`; successive heartbeats that find the prior worker still alive skip and exit, so long sweeps don't pile up. Updates apply to disabled plugins too so re-enabling one uses the latest cached version. Project-agnostic by design: a janitor armed in *any* project updates user-scope plugins for *every* project on the machine, because user-scope is global state. Silent on stdout. Logs every spawn/skip + per-plugin CLI output to `.janitor/logs/user-plugins-update.log`. |
...
| `marketplace_refresh_interval` | 300 | Min seconds between bulk marketplace refreshes. 5 min by default — runs every heartbeat. The detector spawns `claude plugin marketplace update` (no args) detached and returns immediately; the PID is tracked so successive fires that find the prior refresh still in flight log a skip and exit without piling up concurrent fetches. |
