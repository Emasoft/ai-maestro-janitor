...
- **compact:** Guard 2 reads both band edges from one env and this session's own transcript; skip outcome documented (TRDD-PH8SAQKS) (763b28e)
- **rules:** The memory-marker STATE_DIR recipe falls back to the cwd, so a workspace folder with no git no longer refuses the memory job (ac39cf1)
- **terminal:** On an iTerm osascript timeout, log the load average and retry once targeting the session directly (TRDD-L32WC0H7) (5ae6b9b)
- **clear:** An unmeasurable context never satisfies a clear gate; every decision logs its reading; a pending recovery vetoes any automatic clear (TRDD-L32WC0H7) (09c3315)
- **terminal:** Prove the direct iTerm session script live and harden the retry's result handling (TRDD-L32WC0H7) (e74ead1)
- **terminal:** An empty pane read on the retry leg is unread, not empty; the logging fallback cannot raise (TRDD-L32WC0H7) (02f1354)
- **clear:** Measure a resumed session from its transcript before refusing; one recovery veto for every lane (TRDD-L32WC0H7) (bc9f62b)
- **jev:** The health stamp records the failure kind; only an unavailable endpoint declines compactions; budget errors exit 7 (TRDD-RAEGS1D5) (1e36e9b)
- **clear:** A resumed session's transcript reading is stale only if the transcript changed after it, never by age (TRDD-L32WC0H7) (7784344)
- **jev:** Tell an endpoint outage from a local transport failure; the health stamp is written atomically and read defensively (TRDD-RAEGS1D5) (40cc06d)
- **jev:** A 429 is a per-key rate limit with a short pause, not an outage; failure detail lives on the public error class (TRDD-RAEGS1D5) (b171d33)
- **detector:** A docstring line 'word: text' is not a symbol definition; the ordinary-words test uses a hermetic repo (48098ea)
- **detector:** Bare annotations are definitions again; prose is rejected by the type shape, not by requiring '=' (3e16096)
- **detector:** Symbol history looks only at code files; a trailing comment does not hide a definition (35d2284)
- **jev:** The cleared session composes and injects its own Jev context, never another session's handoff (TRDD-RAEGS1D5) (b0e3e6a)
- **jev:** Every automatic clear stamps the shared cooldown once at spawn; a pending recovery vetoes the /clear (TRDD-RAEGS1D5) (2f463d3)
- **jev:** The compacted context lists at most 40 elided pointers plus a count line (TRDD-RAEGS1D5) (9f7c86f)
- **jev:** The new session finds the cleared transcript's record on iTerm; the injection fits the 10 KB hook-output limit (TRDD-RAEGS1D5) (b058292)
- **jev:** The cooldown is spent only by a clear that reaches Enter; stale recovery flags stop vetoing; reload shrinks get the Jev context (TRDD-RAEGS1D5) (1b5ceec)
- **jev:** The injected compacted context fits the hook-output limit by design and keeps the newest, highest-scored items (TRDD-RAEGS1D5) (c406c9b)
- **jev:** The clear chain does not type /clear into a pane the model-fallback actuator is handling (TRDD-RAEGS1D5) (29dd550)
- **jev:** The full compacted context stays on disk; only the injected copy is capped to the hook-output limit (TRDD-RAEGS1D5) (d3364c0)
- **jev:** Stale recovery flags age out, a successful turn proves recovery, and /clear waits while the model window is exhausted (TRDD-RAEGS1D5) (6eba6f5)
- **jev:** A stale resume flag blocks automatic clears for minutes, not a day, in a session nobody resumes (TRDD-RAEGS1D5) (9dfc409)
- **jev:** No startup path injects more than the 10 KB hook limit; the capped copy says read the full file only if needed (TRDD-RAEGS1D5) (eba1f1f)
- **jev:** OpenRouter 402/403 raise JevAuthError, not a malformed-request error (TRDD-541CBN36) (0b88337)
- **jev:** Task notifications and heartbeat turns are not the user's words (TRDD-RAEGS1D5) (2353a88)
- **jev:** The fallback kills llm-ext's whole process group, never feeds it uv-missing or no-digest failures, and marks its prose (TRDD-RAEGS1D5) (8950ba1)
- **jev:** One record classifier decides who wrote each transcript line; heartbeat-driven work and mid-turn owner messages are kept (TRDD-RAEGS1D5) (b215483)
- **handoff:** The pre-compact handoff's recent turns hold only real exchange, not task notifications or command wrappers (TRDD-91D2VHW3) (f6f3394)
- **jev:** The owner's typed slash commands count as the owner; an unknown record origin never does (TRDD-RAEGS1D5) (4a65338)
- **jev:** The llm-ext fallback is typed, reaped after its group is killed, and does not crash on Windows (TRDD-RAEGS1D5) (b0e94f4)
- **clear:** The recent-turns tail injected after a clear shows the owner's latest message, not heartbeats and notifications (TRDD-0UQSAFCW) (6f88b48)
- **jev:** Slash commands the janitor types itself are not the owner's words (TRDD-RAEGS1D5) (d8c80b1)
- **jev:** Every janitor command is system, matched by prefix, and a test proves it for all shipped commands (TRDD-RAEGS1D5) (c5eac23)
- **handoff:** The pre-compact handoff fails loudly on a broken install and always shows the owner's last message (TRDD-91D2VHW3) (7e6e801)
- **clear:** The injected recent-turns tail always leads with the owner's last messages, however far back they are (TRDD-0UQSAFCW) (fda162d)
- **handoff:** A broken classifier import records a finding and still writes the handoff; a timed-out search says it timed out (TRDD-91D2VHW3) (0e5014c)
- **jev:** A batch the provider's firewall blocks, or one too large for the backend, is split and retried instead of killing the compaction (TRDD-1ETALGDG) (c3fdc0e)
- **jev:** Blocked items stay visible and never sink a mostly-good compaction; llm-ext output carries a true header (TRDD-1ETALGDG, TRDD-RAEGS1D5) (434530a)
- **jev:** The injected compacted context carries kept items, the owner's newest message first, not just pointers (TRDD-RAEGS1D5) (fe38e09)
- **jev:** The token budget keeps relevant tool and assistant items, not only the owner's messages (TRDD-RAEGS1D5) (d4fa768)
- **jev:** Each owner message is trimmed, not the owner's share, so stated decisions stay in the compacted context (TRDD-RAEGS1D5) (23713d5)
- **memgrep:** No write verb can put a control byte in a memory page, and lint reports one as an ERROR (TRDD-XI10BA5D) (8476be9)
- **memgrep:** Control-byte follow-ups -- a C1 cause line, repair outside atoms, lint keeps going (TRDD-XI10BA5D) (322a85c)
- **jev:** The full compacted copy names every decision the owner stated, up to 400 (TRDD-RAEGS1D5) (0cf4038)
- **jev:** The injected summary is sized to the room the handoff actually has, so it is never cut by the byte slice (TRDD-RAEGS1D5) (012b091)
- **heartbeat:** A quiet fire says how many background workers are running, and flags one that stalled (TRDD-I63GQJTK) (ac751f1)
- **jev:** A busy handoff trims the card list before the summary, and the summary budget never drops below 800 bytes (TRDD-RAEGS1D5) (44fdec8)
- **jev:** A busy handoff keeps every in-flight card id, and the summary budget never exceeds the real room (TRDD-RAEGS1D5) (aa03812)
- **jev:** The owner's newest message is always in the compacted context, and the injected copy carries at least 3 work items (TRDD-RAEGS1D5) (aabd8b0)
- **jev:** Compose never renders more than its byte budget, down to a one-line minimum that keeps the owner's newest message (TRDD-RAEGS1D5) (8c5db5e)
- **memgrep:** --retire-atom stamps superseded-by whenever it is missing, not only when the atom has no status (TRDD-XI10BA5D) (4a2082a)
- **jev:** The decision log redacts secrets, is private to the owner, writes once per run, logs a session once, and keeps 60 MB (TRDD-N9LDHF7N) (e904477)
- **jev:** An empty injected summary is reported, and when space runs out the context keeps three work items before the owner's older messages (TRDD-RAEGS1D5) (862d30d)
- **jev:** The decision log redacts the whole text before cutting it, and catches passwords, bearer tokens and private keys (TRDD-N9LDHF7N) (a07473b)
- **jev:** The decision log catches credential URLs, JWTs, api_key-style names and secret blobs, and a log failure never stops a compaction (TRDD-N9LDHF7N) (59e782e)
- **jev:** The injected copy picks its items by priority within its own byte budget, so the backstop no longer decides it (TRDD-AW4XD53Q) (c560ca7)
- **rotator:** Capture leaves state untouched when the account lookup fails (TRDD-V6USCGC9) (88b1029)
- **jev:** The compacted context prints the transcript path once, in the expand trailer (TRDD-EFA4P42B) (feae615)
- **jev:** A bare resume, continue or argument-less /compact is an event, not the owner's message (TRDD-DZ1KOGAC) (cfdeace)
- **jev:** The list instruction says replace <id>, and the expand skill names the trailer (TRDD-EFA4P42B) (8cd3d8d)
- **jev:** A control word with trailing . or ! is still a control word; pin the newest-owner fallback (TRDD-DZ1KOGAC) (e5df98a)
- **jev:** The compacted context lists the cards the session worked, and still names every other open card (TRDD-O2FNJ4KW) (873a103)
- **handoff:** An unverified legacy agent-handoff.md gets a pointer, never its body (TRDD-4P4Y2KBR) (5fba6c6)
- **jev:** Card ranking ignores bulk id lists, counts bare ids, and the other-cards line gets its own section (TRDD-O2FNJ4KW) (fdf2de0)
- **jev:** The injected copy is chosen by one reservation-first fill over exact item costs (TRDD-BLGZTHQ9, TRDD-U6C3YXEL) (f87a1a7)
- **jev:** An empty-args wrapper of a content-free command is a control input, not an owner message (TRDD-DZ1KOGAC) (825e5da)
- **jev:** An injected task-notification shows its summary and result, never its id/path wrapper, and a bare tool call fails the result gate (TRDD-BLGZTHQ9) (f2e6ead)
- **jev:** An assistant status update listing many cards no longer skews the card ranking (TRDD-O2FNJ4KW) (687d3dd)
- **jev:** An injected task-notification is shown inline only when its shown excerpt passes the content gate, and its label names only what survived the cut (TRDD-BLGZTHQ9) (1fe0383)
- **hooks:** Give the prompt-submit hook the same 10 s budget as its siblings (74254d1)
- **jev:** Join attachment.prompt block list instead of assuming str (TRDD-RAEGS1D5) (af09571)
- **handoff:** Join a pasted-image prompt's text blocks in the recent-turns tail (TRDD-RAEGS1D5) (b857a97)

### Documentation

- **trdd:** Card guard 2's wall-reaching upper edge, fix new cards' attribution and column (TRDD-PH8SAQKS, TRDD-PPQG9JVP) (eadb153)
- **memory:** Split the oversized publish gate-sequence atom into four (4bb94cc)
- **memory:** Split four oversized macos-keychain atoms into eight size-bounded ones (210753e)
- **memory:** Repair six truncated or over-cap atom descriptions on macos-keychain.md (ebe664f)
- **memory:** Restore the symptom phrases three macos-keychain descriptions lost in ebe664f9 (d310413)
- **trdd:** Open the Jev compaction program (TRDD-RAEGS1D5, TRDD-541CBN36, TRDD-23QM8H5F) (4d8daa3)
- **trdd:** One STATE block per card, truthful columns, binding design in the parent (TRDD-RAEGS1D5, TRDD-541CBN36, TRDD-L32WC0H7) (012c524)
- **trdd:** L32WC0H7 STATE no longer claims 'held at testing' while in dev; unpark it; RAEGS1D5 relevant-rules explicit (TRDD-L32WC0H7, TRDD-RAEGS1D5) (899c668)
- **trdd:** Delete the empty review-after line on L32WC0H7; file the trddgrep append/unset defects (TRDD-E8DRTDC6) (c44e2d6)
- **memory:** Atomize three PROJECT pages into nine recall-addressable atoms (cb432af)
- **jev:** Comments in the automatic compaction lane describe Jev compaction, not the llm-ext era (TRDD-RAEGS1D5) (82860cc)
- **trdd:** 4JEBTT2C, YM65RCZA and PH8SAQKS superseded by TRDD-RAEGS1D5; 5A4SGMD6 records the #297 close (e550ed5)
- **trdd:** RMX0IE72 and WY198OIP complete with evidence-cited checklists; YONEH3XC and L64C5DQ1 await the owner (abd8ae5)
- **memory:** Split the oversized macos-keychain page into an overview and three sub-pages (8e5c898)
- **memory:** The two new macos-keychain sub-pages are components, not aspects (ea5d74d)
- **trdd:** The safe mechanical board-lint subset — derived flags on 9 cards, one blocked-card fix, one zone move (94257a8)
- **trdd:** ULEGRT01 is not marked derived after all — it has its own EHT (9dea9fc)
- **trdd:** Close 541CBN36 and unblock RAEGS1D5 with a current STATE block (TRDD-541CBN36, TRDD-RAEGS1D5) (d3455d2)
- Add TRDD-IEBZ4JC5 — janitor maintainers and indexers must run in project folders whose git repos live in subfolders (2addfab)
- **trdd:** Record the owner's 2026-09-23 decisions on the Jev lane (TRDD-RAEGS1D5) (33cc7dd)
- **trdd:** Correct the recorded owner decisions and quote them verbatim (TRDD-RAEGS1D5) (5a1e121)
- **memory:** Heartbeat atomize pass on 5 PROJECT wikimem pages (f4da695)
- **trdd:** L32WC0H7 back to testing — card 1 landed, only the F5 live observation remains (TRDD-L32WC0H7) (84be21a)
- **trdd:** L32WC0H7 STATE no longer claims "only F5" remains or that its commit list is complete (TRDD-L32WC0H7) (a691d88)
- **trdd:** L32WC0H7 STATE lines agree on what remains; the pane-capture note no longer reads as superseded (TRDD-L32WC0H7) (4c11066)
- **memory:** Split oversized atom ATOM-SXV6-8JN0 into two one-fact atoms (e106479)
- **trdd:** Record the owner's 2026-09-23 evening Jev directives verbatim (TRDD-RAEGS1D5) (46ace24)
- **trdd:** RAEGS1D5 STATE carries the no-publish condition; owner quotes separated from our evidence (TRDD-RAEGS1D5) (61ffd5f)
- Add TRDD-CC0CZLMO — Jev scores batches sequentially, so large transcripts exceed both lane timeouts; RAEGS1D5 STATE credits only the owner's own release condition (TRDD-RAEGS1D5, TRDD-CC0CZLMO) (0c277fb)
- Add TRDD-0UQSAFCW, 91D2VHW3, HWF3QFAB, 88DOI824, N9LDHF7N — the Jev reference-adoption cards (TRDD-RAEGS1D5) (9fa551d)
- Add the release-blocker card for Cloudflare-blocked and oversized Jev batches (TRDD-RAEGS1D5) (db67eb7)
- **trdd:** The Jev publish blockers are machine-visible on RAEGS1D5; landed cards move to testing (TRDD-RAEGS1D5) (ffebbd1)
- **trdd:** CC0CZLMO is blocked on the split fix, and the Jev EHT cards declare derived: true (TRDD-CC0CZLMO, TRDD-RAEGS1D5) (f36dee8)
- **trdd:** The Jev EHT cards name RAEGS1D5 as parent (TRDD-RAEGS1D5) (4352e6f)
- Fix the two CPV --strict findings that block the release (TRDD-RAEGS1D5) (1ffc6a6)
- **rules:** The STATE_DIR command lives in a code block below the heartbeat table, copyable as one line (TRDD-RAEGS1D5) (f0eb084)
- Add a card so a heartbeat during in-flight background work shows progress, not silence (8e8a07f)
- **trdd:** I63GQJTK states the stall's real cause and a stall threshold that survives long test runs (21bb65a)
- Add the card making memgrep the only writer of wikimem pages (owner directive 2026-09-23) (e8d49ee)
- **trdd:** XI10BA5D records the owner's memgrep write contract verbatim (TRDD-XI10BA5D) (9272a4e)
- **trdd:** XI10BA5D records that memgrep is the writing gate (TRDD-XI10BA5D) (42500c3)
- **trdd:** XI10BA5D records the owner's atom-size decision and the ticket threshold (TRDD-XI10BA5D) (360d240)
- **trdd:** RAEGS1D5 records the owner's release scope: complete everything before publishing (TRDD-RAEGS1D5) (7220774)
- Memgrep verbs are the only allowed way to create or edit a wikimem page (TRDD-XI10BA5D) (d20574f)
- **trdd:** XI10BA5D: one janitor ticket per non-auto-fixable lint issue, no cap (TRDD-XI10BA5D) (8a81891)
- **trdd:** XI10BA5D records the d20574f0 review: 5 skills still hand-edit the staged copy (TRDD-XI10BA5D) (3330598)
- **trdd:** XI10BA5D separates the owner quotes from our readings, lists the migration open questions (TRDD-XI10BA5D) (1bd2c4d)
- **trdd:** XI10BA5D gets a STATE block, moves to verify_assumptions, and splits out three derived cards (TRDD-XI10BA5D) (bfeae47)
- **trdd:** XI10BA5D STATE names what each derived card is (TRDD-XI10BA5D) (daccf3f)
- **trdd:** XI10BA5D STATE separates owner decisions from ours; 6V7ZCKXF leaves the release path (TRDD-XI10BA5D) (c2d60c1)
- **trdd:** Cards stop carrying the owner's macOS login name (TRDD-YONEH3XC) (9b22161)
- **trdd:** XI10BA5D records the twice-reviewed memgrep gate plan and the defaults for four unanswered owner questions (TRDD-XI10BA5D) (d07cf19)
- **skills:** The repair chore fixes a one-sided link and a missing desc through memgrep verbs, before the transaction (TRDD-XI10BA5D) (7f983ec)
- **skills:** Repair's pre-transaction verb fixes use a fresh sha per call, re-diagnose before begin, and never write a second page (TRDD-XI10BA5D) (0e96466)
- **trdd:** XI10BA5D STATE names the recorded defaults and the work in flight; the plan carries every accepted review finding (TRDD-XI10BA5D) (5c9cc1f)
- **trdd:** XI10BA5D STATE drops the stale lower-bound and in-flight claims, and marks the staged-copy proposal superseded (TRDD-XI10BA5D) (4e05213)
- **skills:** Repair takes the base sha from the bytes it decided on, and refuses an empty sha (TRDD-XI10BA5D) (2f04d1b)
- **skills:** Repair's empty-sha guard actually skips the verb (TRDD-XI10BA5D) (7d0127a)
- **trdd:** RAEGS1D5 records the publish acceptance criteria and corrects two "measured" claims; 98SP58TJ carded (TRDD-RAEGS1D5) (0e5d02a)
- RAEGS1D5 STATE names what landed and what is next; criteria count absences from the compact run; the repair skill guards its real link write (TRDD-RAEGS1D5) (7cc1b27)
- **trdd:** RAEGS1D5 STATE names the full pre-publish path and marks superseded bullets against the code (TRDD-RAEGS1D5) (fc47c92)
- **trdd:** RAEGS1D5 labels only the current step NEXT ACTION (TRDD-RAEGS1D5) (3bcea8a)
- **memory:** The review-gate page records that a commit after the review fork re-arms the gate (TRDD-XI10BA5D) (e2c8f12)
- **memory:** Correct the review-gate lesson that overstated the gate (TRDD-XI10BA5D) (183ab29)
- Mark the overstated review-gate lesson as corrected; XI10BA5D records the stale-PATH memgrep release step (TRDD-XI10BA5D) (c739079)
- **trdd:** XI10BA5D notes that a page with several control bytes needs one write that fixes them all (TRDD-XI10BA5D) (eb3e3af)
- **trdd:** XI10BA5D records the add-atom --supersedes guard bug found beside the retire fix (TRDD-XI10BA5D) (6e4e8a5)
- **trdd:** RAEGS1D5 records that the injected copy dropped from 4/6/4 to 3/3/3 work items; the priority-admission fix is carded (TRDD-RAEGS1D5) (e30b088)
- **memory:** Trim an over-cap atom description on the Cloudflare 1010 page (janitor repair chore) (f35e593)
- **trdd:** AW4XD53Q is a release blocker, not backburner (TRDD-RAEGS1D5) (5f0f539)
- **trdd:** AW4XD53Q body states it is a release blocker (TRDD-RAEGS1D5) (2fc2047)
- **trdd:** RAEGS1D5 NEXT ACTION names the AW4XD53Q release blocker (TRDD-RAEGS1D5) (3743e77)
- Add TRDD-K0PMVRN6 — all three rotator slots died with invalid_grant and rotation stayed put (2ef3b1f)
- Record the owner rotator decisions and the ratified renew procedure (TRDD-K0PMVRN6) (b93c803)
- Add TRDD-FWDZDB7W and replace the host username on three new cards (TRDD-FWDZDB7W) (e2757bf)
- K0PMVRN6 states it still carries personal addresses (TRDD-K0PMVRN6) (c9c0f4e)
- Add TRDD-WZKFSQ2N — the janitor owns continuity and proves each responsibility works (7523169)
- File the seven gaps from the responsibility audit (TRDD-WZKFSQ2N) (ca9d990)
- Correct the rotation cards after the keychain read test and record the owner decisions (TRDD-4XND73XD) (ff236a1)
- Record the owner delegation and the continuity decisions; close AW4XD53Q (TRDD-WZKFSQ2N) (7e43736)
- The card updates that 7e437369 described but did not contain (TRDD-WZKFSQ2N) (54f7a2b)
- Record the capture stopgap, the hotfix outcome and the AW4XD53Q closing evidence (TRDD-K0PMVRN6) (ab477a2)
- Move the capture stopgap into K0PMVRN6 STATE, out of the verbatim owner section (TRDD-K0PMVRN6) (fa9b4a8)
- The capture stopgap skips after a real switch, and the switch is watched (TRDD-K0PMVRN6) (3ff01e5)
- Card the Jev injected-copy defects and the release-1 rotation items (TRDD-RAEGS1D5) (35533da)
- Apply the advisor's root causes and two reviews to the new cards; link them as release blockers (TRDD-RAEGS1D5) (2cf13ad)
- Supersede contradicted card lines in place and make the release card's facts true (TRDD-RAEGS1D5) (ded353f)
- Record feae6152 on EFA4P42B and finish the small card corrections (TRDD-RAEGS1D5) (b9b870c)
- Record the landed Jev fixes, move them to testing, and card the verification matrix (TRDD-RAEGS1D5) (5bea0b3)
- Record the f87a1a79 deviations, the 13:17 stale-handoff recurrence and the AgentlensPro memgrep reports on their cards (TRDD-RAEGS1D5) (9717e9f)
- Bring the Jev cards current with f2e6ead7..d736d1f3 and the stale-handoff replay (TRDD-RAEGS1D5) (0b9fcc8)
- **skills:** Move repair's pre-transaction mechanics and checklist detail to references, back under the 5000-token body cap (1854634)
- Card the verbatim-prose design with the advisor's fixes, plus its two follow-ups (TRDD-D7RLXAN1) (d13734e)
- D7RLXAN1 waits on the owner, with its three open questions and its facts corrected in place (TRDD-D7RLXAN1) (69305cf)
- **skills:** Restore the repair steps 1854634c condensed away; pure-move four background blocks (still 800 tokens over the cap) (4fb3529)
- **skills:** Move repair's conditional pre-transaction verb-fix procedure behind a mandatory read, under the token cap (TRDD-XI10BA5D) (b5e1983)
- **skills:** Keep the verb fixes' dry-run and sha guards next to the required read (TRDD-XI10BA5D) (4f1857c)
- The 15:21 live-slot capture was skipped because the rotator switched at 13:32 (TRDD-K0PMVRN6) (8734872)
- K0PMVRN6 records the 13:32 pre-wall switch as observed and narrows the stopgap result (TRDD-K0PMVRN6) (cefca2d)
- Point the cards at the rewritten commit ids (mechanical, TRDD-K0PMVRN6) (5891ea3)
- **skills:** Fix the repair skill's moved links and restore the TOCs CPV requires (TRDD-XI10BA5D) (965d3ce)

### Features

- **jev:** Vendor jevctx@6d33376, add the OpenRouter Jev provider and the jev_compact scorer CLI (TRDD-541CBN36) (4b8ba76)
- **jev:** The compacted-context composer — extract, digest, score, compose (TRDD-RAEGS1D5) (6fd3e53)
- **jev:** Jev_compact compact — score the old transcript and write the compacted context; janitor-expand skill (TRDD-RAEGS1D5) (522f1e3)
- **jev:** The compaction lane composes the compacted context through jev_compact; every outcome becomes a finding (TRDD-RAEGS1D5) (83fae09)
- **jev:** Retry Jev for up to 5 minutes, then fall back to llm-ext; no failure pauses compaction for 30 minutes (TRDD-RAEGS1D5) (88aea2d)
- **jevctx:** Vendor segments, shadow and pipeline from the reference jev-compaction (TRDD-RAEGS1D5) (49d733b)
- **jev:** Score batches in parallel and use the reference's retrieve question, pointers and per-kind batching (TRDD-CC0CZLMO, TRDD-HWF3QFAB) (61cad99)
- **jev:** Large tool results are split into exact segments, so more of them fit the budget (TRDD-88DOI824) (3006e92)
- **jev:** Every keep-or-drop decision is logged, and replay shows what another threshold would do (TRDD-N9LDHF7N) (a9b338c)
...
- **jev:** Probe off the session-start path with a cached stamp; trim the vendored set to the scoring half; pin the subprocess boundary (TRDD-541CBN36) (0930d39)
- **jev:** No network probe at arm time; the boundary test covers every shebang script (TRDD-541CBN36) (54cebc4)
- **jev:** The automatic compaction lane no longer contains llm-ext; the lane logic lives in a library module; the hold releases on the compacted context (TRDD-RAEGS1D5) (2c32448)
- **jev:** Every automatic shrink is a Jev compaction; the janitor never types /compact or a handoff skill (TRDD-RAEGS1D5) (54ea73b)
- **jev:** Delete what retiring /compact left unused — proactive-idle helpers, the queued-compact cancel, four plugin options (TRDD-RAEGS1D5) (fa6ceed)

### Testing

- **compact:** Pin guard 2's settings-merged upper edge and transcript-path preference; stop claiming when the harness compacts (TRDD-PH8SAQKS) (e2ae256)
- **rules:** Pin the heartbeat rule's STATE_DIR recipe to state.py's lookup (1d5ec8b)
- **rules:** Isolate the STATE_DIR recipe test from a leaked git environment and prove it can fail (a6b08e2)
- **detector:** The real-repo regression test also asserts a symbol added today is found through the code-file pathspec (c749688)
- **jev:** The compaction entry script runs stdlib-only under uv run --script; the automatic lane imports nothing from the manual summarizer (TRDD-RAEGS1D5) (eb49e93)
- **jev:** The runtime test proves the stub composer ran from CLAUDE_PLUGIN_ROOT; user site-packages excluded; the manual-summarizer import ban covers tests too (TRDD-RAEGS1D5) (ef06432)
- **jev:** The manual summarizer's tests live with the manual summarizer; the automatic-lane test file no longer imports it (TRDD-RAEGS1D5) (d67b10c)
- **jev:** The Jev test files pass pyright; the boundary allow-list states why the manual summarizer import stays (TRDD-RAEGS1D5) (f4bc954)
- **jev:** The clear-sidecar pane-key test checks writer and reader agreement, not terminal_pane_key (TRDD-RAEGS1D5) (051625a)
- **memgrep:** Retiring an atom leaves a quoted desc with commas, brackets and links byte-identical (TRDD-XI10BA5D) (f37240a)
- **jev:** The wording test reads the instruction lines the render actually prints (TRDD-EFA4P42B) (2fe1966)
- Read the STATE_DIR recipe from the rule's code block, where f0eb0848 moved it (TRDD-RAEGS1D5) (7f421d9)
- **jev:** Pin the card-mention cap at its exact boundary and who it exempts (TRDD-O2FNJ4KW) (ad8a82b)
- **jev:** Pin the full notification label when a real result survives (TRDD-BLGZTHQ9) (d736d1f)
- **jev:** Fragment the shadow-log redaction fixtures so the secret-literal hygiene gate passes (TRDD-N9LDHF7N) (880d318)

### Memory

- Atomize the memgrep links --to/--from page into one fact atom (I3P7THOS) (a2fbf47)
## [3.5.7] — 2026-09-17
...
- **git_utils:** Route all four probes through the timeout_scale seam (9c8e9a0)
- **tests:** Clear the 7 pyright errors CI rejected on 3.4.14 (e552c1d)
- **publish:** The gate now runs pyright and lints tests/ (TRDD-MYQGMAQZ) (308bc49)
- **publish:** Pyright fails CLOSED in the gate, and the probe goes with it (82944b5)
- **publish:** Raise the pyright timeout 300s -> 900s (2fc92cc)
- **publish:** Correct every false "parity with CI" claim in the gate (3bf6930)
- **publish:** The skip warnings told the operator CI would catch it — it won't (aea0483)
- **publish:** Never ship a tag that names a stale commit (TRDD-S7FIQTCO) (397c378)
- **publish:** Close the false greens adversarial review found (TRDD-S7FIQTCO) (93bd6c1)
- **pre-push:** Refuse the release gate under a sub-3.11 python3 (TRDD-CN62E66F) (8662094)
- **pre-push:** Absent python3 gets its own refusal, not the "too old" one (8791091)
- **pre-push:** The version refusal no longer states a fact it has not established (9ee4f43)
- **pre-push:** One-line the refusal; the .git/hooks copy is a FALLBACK, not dead weight (3f6c570)
- **8BXMNQ4T:** 258d4423's measurement was wrong — p99 BREACHES the 60s ceiling (f588533)
- **8BXMNQ4T:** Withdraw f5885338's cause-attribution; stall is attributed and load-correlated (5bd5907)
- **8BXMNQ4T:** Thesis CONFIRMED — all 12 stalls pair with a long foreground body (5f65854)
- **8BXMNQ4T,FFXGPZEI:** Per-row check refutes "all 12 pair"; public correction on #297 (b46c645)
- **8BXMNQ4T,QJ5LP4W2:** Break the split's deadlock; STATE said "confirmed" over a body saying it is not (000d957)
- **branch-protection:** The setup skill's convergence check was NAME-only (janitor#294 defect 2) (c984c54)
- **tests,BH32A1A5:** Answer the env question with a command; restore the arity check (0eeec2f)
- **tests,#294:** Delete discard lines that satisfy nothing; revert another agent's issue body (6097da7)
- "testing/blocked untouched all session" was FALSE; trim the pyright comment to what ran (9da91e8)
- **5EHBPH6G:** Supersede a STATE block whose two premises both came true; correct my own session-boundary claim (2a60ee9)
- **5EHBPH6G:** All 211 deferrals are ONE plugin retried, not 40 iterated — my own rebuttal was wrong (b388256)
- **5EHBPH6G:** APPLY the clause-2 rewrite to the box, not to prose above it (bdcad29)
- **5EHBPH6G:** The "retry loop" side-question dissolves on scoping — no card, no defect (945fb3e)
- **5EHBPH6G:** Clause 2 was unsatisfiable AGAIN — replace the absence with a BOUND (81bf04c)
- **5EHBPH6G:** V3's threshold measured the wrong quantity — and the measurement found a 22-min wait (49b9b13)
- **5EHBPH6G:** The 22-min wait is PRE-FIX — all four boxes pass, card is complete-eligible (a2893c6)
- **5EHBPH6G:** Prove the pre-fix attribution by BEHAVIOUR; make the exit condition satisfiable (c8ad909)
- **5EHBPH6G:** Fill implementation-commits; move the surviving condition to PROJECT wikimem (4927cf0)
- **memory:** Scope the host figures in ATOM-PLJ6-NM0P — PROJECT memory is PUSHED (9f93a24)
- **daemon:** Log the pane class on FIRED/REFUSED, so KE88RIKX box 6 can be read at all (6665888)
- **trdd:** Bump `updated:` on the two cards I edited substantively without it (dc11dc5)
- **guard:** Branch protection resolves the repo from the PROJECT root only (TRDD-BH32A1A5) (e4dd674)
- **trdd:** BH32A1A5 pre-block-column was FABRICATED — dev -> complete; verify unblock-when parses (6e626b9)
- **daemon:** Marketplace-refresh stops chasing unregistered and local marketplaces (a15fa65)
- **daemon:** Keep the `refreshed N/M` line honest about what the filter skipped (870eab1)
- **daemon:** Empty-plan message stops claiming "none installed" after a filter (26e221c)
- **tests:** Scale the subprocess ceilings conftest's Popen patch cannot reach (TRDD-Q8PNPRTW) (5b5267a)
- **docs:** Blank line after the table in TRDD-8BXMNQ4T — unblocks stage 4 of publish (d4e5f05)
- **trdd:** Redact the home path this card leaked — it had become a WORSE publish blocker (7541ba3)
- **trdd:** TRDD-Q8PNPRTW — row 2 is an ORDERING DEFECT in the test, not an undetermined race (a081d46)
- **trdd:** TRDD-Q8PNPRTW — ROW 1 FAILS TOO, which kills all three of my diagnoses (db466c6)
- **trdd:** TRDD-Q8PNPRTW — both mechanisms for rows 1 & 2 are measured false (042b7dd)
- **trdd:** TRDD-Q8PNPRTW — two of the three legs under "env={} is dead" were void (103f638)
- **trdd:** TRDD-Q8PNPRTW — the STATE block's head was stale and its own middle refuted it (397b63c)
- **trdd:** TRDD-Q8PNPRTW — retract "10 never triaged"; the triage existed and I never looked (4f5f11e)
- **trdd:** TRDD-Q8PNPRTW — measured rows 1&2: 50s budget, file never written, cause undetermined (c5d78d9)
- **trdd:** TRDD-Q8PNPRTW — the shell never runs; suite is red with a DIFFERENT 11 (98c1717)
- **trdd:** TRDD-Q8PNPRTW — branch_protection is a load artifact; e4dd674d exonerated; 3 overclaims retracted (4b187b3)
- **trdd:** TRDD-Q8PNPRTW — retract "exonerated"/"settled"; dev -> todo (085089e)
- **trdd:** TRDD-Q8PNPRTW — todo was wrong too; blocked on a USER decision, and the head had drifted again (f5d576f)
- **trdd:** TRDD-Q8PNPRTW — the split left it UNCLOSABLE; narrow the criteria, name the real blocker (7af5662)
- **trdd:** The split COPIED rows 1&2 instead of moving them; mark the copy, narrow an over-claimed exclusion (6cb3f91)
- **trdd:** TRDD-Q8PNPRTW — blocked was wrong; human_review is the honest column (1c54a9f)
- **trdd:** TRDD-Q8PNPRTW — bump updated: to 12:30, matching the column-change bullet (014fc37)
- **trdd:** TRDD-Q8PNPRTW — min-approval-requirement none contradicted human_review (2799350)
- **trdd:** TRDD-Q8PNPRTW — the waiver ask is to RELAX the criterion, not satisfy it (ecf6abf)
- **trdd:** TRDD-Q8PNPRTW — the cause IS named; measurement replaced the waiver ask (36ca4a6)
- **trdd:** Retract "not host load"; record fd-exhaustion as the hypothesis that explains BOTH cards (1dce884)
- **trdd:** TRDD-K7WQ2NRB — hypothesis 5 dead in 20 minutes; withdraw the two-cards-one-bug framing (27d5cc8)
- **trdd:** TRDD-K7WQ2NRB — I over-retracted; one live sub-hypothesis went out with the dead one (001fb75)
- **trdd:** TRDD-K7WQ2NRB — NEXT ACTION was stale; stderr capture is step 1, _enforce_spawn demoted (5fc15b0)
- **trdd:** TRDD-Q8PNPRTW — the decision line asked the USER to sign a mechanism that is not one (df158d8)
- **trdd:** TRDD-3BQM5GH7 — the ignore file is for a tool we do not run (595a3a9)
- **trdd:** TRDD-QJ5LP4W2 — I cited a function that does not exist, and framed the evidence toward the wrong remedy (c74607d)
- **trdd:** TRDD-QJ5LP4W2 — off by one, self-contradictory, and I over-corrected the brief (da1040b)
- **trdd:** TRDD-QJ5LP4W2 — stop reaching for a mechanism the evidence does not pin down (4453f8b)
- **trdd:** TRDD-QJ5LP4W2 — the prose learned the lesson, the table did not (378fc8c)
- **trdd:** TRDD-74AA4PAL, TRDD-OES0NN3F — I under-classified the card I wanted to ship (399996b)
- **hooks:** TRDD-OES0NN3F — inject the handoff after a compaction, not just after /clear (42a24e6)
- **hooks:** TRDD-OES0NN3F — the injection I shipped 12 minutes ago compounds, and its age bound disagreed with dispatch (0b4f72c)
- **hooks:** TRDD-OES0NN3F — restore the 24h bound I broke, and deliver before recording (2c852b5)
- **hooks:** TRDD-OES0NN3F — a compact-path function should not read a clear-named option (b7fa08b)
- **hooks:** TRDD-OES0NN3F — restore the disable hatch, and stop naming an encoding trigger that does not fire (34b0d74)
- **clear:** TRDD-L46IG69Y — the concision warning fired on every composer handoff (bf55362)
- **cadence:** TRDD-2640RYR5 — a directive's mtime was measured against the caller's clock (b535dfc)
- **board:** TRDD-AM8JD9SG gets the unblock-when ZQ02QG1L lacked — and my first example of it was malformed (43cd9ec)
- **clear-lane:** The automated resume directive never asked for the after-phase (TRDD-1QJIZFFW) (e9cedc8)
- **clear-lane:** The after-phase path was repo-relative and would have failed on every project but this one (TRDD-1QJIZFFW) (f514de4)
- **clear-lane:** ASCII-only directive, quote-terminated path regex, and the version-pinning limit stated (TRDD-1QJIZFFW) (b965f7e)
- **terminal_trigger:** Drive iTerm through the verified injector — the blind self-sender typed /compact four times into one field (TRDD-HMLS5WE8) (6803ade)
- **terminal_trigger:** The self-send ceiling refuses under a 5 s floor, one clock read per command; wikimem atom for the incident (TRDD-HMLS5WE8) (0c7037b)
- **terminal_trigger:** Key the self-send lock and dedupe stamp by PANE, not by project (TRDD-HMLS5WE8) (a045540)
- **post-compact-resume:** Defer the resume push on an attended pane and re-check it, instead of dropping it (TRDD-74AA4PAL) (ff6a781)
- **safe-storage:** A stalled security call is not a keychain denial — may_prompt gate, 3-consecutive-timeout latch, slow-op log (TRDD-3VIXO8FA) (2be7827)
- **daemon:** Floor tasks dispatch first, budget counts float seconds (TRDD-QJ5LP4W2) (57a7f26)
- **daemon:** Floor statement takes the gate's own ownership test, a failed memory write is logged, the test pins its control dir; A70YJLXN dev -> testing (TRDD-A70YJLXN) (20753ca)
- **claudemd-slim:** Corpus_digest hashes the rendered index body, not a hand-picked field pair (TRDD-Q3WSQ9M5) (4604835)
- **memory-dispatch:** Never print a [janitor-memory-<chore>] marker whose claim pool is empty — write-time read-back + relay-time gate (TRDD-LDSCQ0NU) (9681eb7)
- **dispatch:** A suppressed [janitor-memory-<chore>] marker is recorded in the findings ledger; docstring no longer claims a machine-wide pool; relay gate proven scoped to memory-maintenance (TRDD-LDSCQ0NU) (61dcfde)
- **trdd_common:** Extract_uid accepts the bare v1-migrated TRDD-<id8>-<slug>.md shape (TRDD-JDIJ76SW) (5ea6375)
- **tests:** Conftest _source_manifest prunes target/ and __pycache__/ during the walk instead of after it (TRDD-TASA9ACJ) (0f2edc7)
- **tests:** _source_manifest keeps is_file() and the walk-spy test asserts os.walk was actually used; docstring names the two by-design differences from rglob (TRDD-TASA9ACJ); Q8PNPRTW records the 13 tracebacks — 12 byte-stable, inject_still_wanted fails in isolation (TRDD-Q8PNPRTW) (62bbad0)
- **tests:** Test_inject_still_wanted supplies the three pane reads inject_until_sent's confirm makes and asserts the reader is then exhausted (TRDD-KS41G6AL); HMLS5WE8 STATE records the bisect (green at 6803ade0, red from 0c7037bc); Q8PNPRTW adds the defang order-dependence data point (TRDD-KS41G6AL, TRDD-HMLS5WE8, TRDD-Q8PNPRTW) (7b4148a)
- **memory-maintenance:** _write_pending supersedes the unclaimed record for the same key by rename and the superseded prefix joins the keep-20 prune; claim carries the scheduler's state_dir and refuses on a resolved-path mismatch, --state-dir '' exits 4, no state at all exits 3 (TRDD-Q7X4M2KP, TRDD-N1CPV1QV); LDSCQ0NU's DETECTOR_TIMEOUT guard; add TRDD-HXZ8B0IS (TRDD-LDSCQ0NU, TRDD-HXZ8B0IS) (af6340a)
- **state:** _log_fail_open also prints one [run_subprocess] line to stderr so a fail-open None is visible to a heartbeat reader (TRDD-9EAQS97B); Q7X4M2KP -> testing; HXZ8B0IS transition-only; Gatekeeper hypothesis refuted (8bcd297)
- **memory-skills:** The spawn turn composes STATE_DIR and passes it in the prompt; all 8 memory-chore SKILL.md claim commands take --state-dir behind a ${STATE_DIR:?} guard; exit codes 3/4/5 documented (TRDD-N1CPV1QV) (69adc82)
- **memory-skills:** Export STATE_DIR from the spawn prompt before the ${STATE_DIR:?} guard in all 8 memory-chore skills; retro-lesson claim step becomes a fenced block; ordering test (TRDD-N1CPV1QV) (857bd7b)
- **orphaned-memory-maint:** The detector enumerates the per-dispatch pending pool through memory_dispatch_claim.candidates() and applies the same orphan rule to every unclaimed record, keyed pool:<dispatch_id>; legacy slot unchanged (TRDD-IB5B14QQ) (028de46)
- **memory-claim:** Any state dir with no memory-maint-* files exits 3, explicit --state-dir included; the skills' export placeholder is an empty string so a verbatim paste dies at the guard (TRDD-N1CPV1QV) (6fbb8c8)
- **memory-claim:** Exit-3 line also names the never-dispatched case; IB5B14QQ STATE states the per-record measurement (9 not-current, 4 below cadence x factor, 1 orphan) instead of one unmeasured cause (TRDD-N1CPV1QV, TRDD-IB5B14QQ) (08eaec9)
- **daemon:** The tick takes ONE liveness read — exit gate, yield decision and transition log all derive from the same probe; non-object JSON is malformed, not read-error; 89 s boundary test pins its clock (TRDD-ARTTXA7P, TRDD-HXZ8B0IS) (a02c58d)
- **memory:** WQBHAC45 regains the `fact` subcommand on its recall surface; the repair skill's desc-trim rule now requires every cut clause to survive in keywords (3f17634)
- **branch-protection:** Trace every gh-reader soft-fail in branch_protection_lib to stderr (TRDD-PJD6XV66) (eb4bce2)
- **model-fallback:** Only a true 100% window qualifies, rotate first when a sibling has headroom, back off 1h after a cancelled switch (TRDD-M4HVFU2A) (772e46a)
- **model-fallback:** Rotate-first stand-down shares rotate_to's headroom predicate and names the no-arg verb (TRDD-M4HVFU2A) (2d8cf86)

### Documentation

- **memory:** Record the two publish-gate lessons 3.4.14 cost (8c97d1e)
- Add TRDD-S7FIQTCO — publish recovery reuses the stale tag (0590dea)
- Close NACCL0CB, prove the daemon runs 3.4.14, correct an overclaim (101f4ca)
- **TRDD-L32WC0H7:** Tick 4 acceptance boxes, mark 2 untestable by nature (9d0a5a8)
- **TRDD-MYQGMAQZ:** Scope the divergence table, note the assert stand-in (d7bc01a)
- Correct two records — the assert count, and a lesson that prescribed a chore (9925bb7)
- **TRDD-MYQGMAQZ:** Record the open policy question, both sides (139de8f)
- **publish:** The pyright ceiling now cites a measurement, not a guess (1507a68)
- **publish:** Retract the "cold-ish measurement" — --refresh measured nothing (7af2e27)
- **publish:** Name why no cold measurement exists — the payload is not uv's (9a27628)
- Retract the runtime-fetch claim, and shrink the comment to what is true (32cc161)
- **TRDD-MYQGMAQZ:** The parity test's blind spot, and a third false parity claim (1111b03)
- **TRDD-MYQGMAQZ:** An orphaned .mega-linter.yml is why the false claims held (445faf4)
- **TRDD-MYQGMAQZ:** Scope-limit the mega-linter claim — I made the error I documented (cc725d3)
- **TRDD-MYQGMAQZ:** Ran the CPV preflight — the same falsehood is upstream (8be0bd7)
- Split three cards off MYQGMAQZ, and qualify the "enforcer" claim (a9501ef)
- **TRDD-6SIY2VX2:** CPV does not read .mega-linter.yml — measured, not asked (991c028)
- **TRDD-6SIY2VX2:** Separate what is a repo fact from what is my laptop (2fce7bb)
- **TRDD-6SIY2VX2:** The config has 12 linters, not 11 — I read a truncated list (368b5fe)
- **TRDD-6SIY2VX2:** Withdraw a circular inference, and verify the count properly (86f6e58)
- **TRDD-6SIY2VX2:** The conclusion survives on better evidence than I withdrew (28c8d17)
- **mega-linter:** Mark the config dormant instead of deleting it (TRDD-6SIY2VX2) (7f6532d)
- Archive TRDD-6SIY2VX2 -> complete (d1422c2)
- Reject the CI-wait reordering on evidence (TRDD-J1KRAY9C -> TRDD-1B7ZYKHR) (2f46717)
- Archive TRDD-J1KRAY9C -> complete (dda9ea3)
- Close MYQGMAQZ + S7FIQTCO, correct a stale verification claim, file CN62E66F (d130c8b)
- Archive MYQGMAQZ + S7FIQTCO -> complete (57f1358)
- **CN62E66F:** Hedge two claims the card stated more strongly than measured (ed6fa9a)
- Reopen S7FIQTCO as EHT-blocked; fix a remedy that does not remedy (1f2f58e)
- **CN62E66F:** The ref-deletion path breaks my own mildness argument (6c838fe)
- S7FIQTCO -> blocked (not testing); verify the gate's pre-change behaviour (e7fa0d8)
- **ZQ02QG1L:** Record two consequences the conversion introduced (f2599bb)
- Close TRDD-ZQ02QG1L; retract a finding one grep disproves (0d2eb66)
- Archive TRDD-ZQ02QG1L -> complete (9bcddc2)
- ZQ02QG1L -> blocked on new EHT L46IG69Y; fix two wrong line citations (72725c0)
- Close 2 of the 4 CODE boxes the testing re-audit found; correct the other 2 (f39a0e8)
- Re-cite the .env box against the test that actually matches its fixture (b72ae93)
- Cite the code body, not the docstring, for the .env box (f74dbfe)
- CN62E66F terminal -> unblock S7FIQTCO; archive CN62E66F (72ac8a0)
- Archive TRDD-S7FIQTCO -> complete (7f45a20)
- **pre-push:** Correct a FALSE claim in 87910911's commit message (b06105d)
- **pre-push:** Drop an untestable version premise instead of hedging it (68824c7)
- **ZQ02QG1L:** The conversion's benefit is ZERO on this host until a release ships (9df68b7)
- **ZQ02QG1L:** RETRACT "the benefit is zero on this host" — it is half live (e9904f0)
- **ZQ02QG1L:** Separate what was MEASURED from the mechanism I guessed (e096ee6)
- **ZQ02QG1L:** Split loader-evidence from file-evidence; schedule the collapse (64ac5c6)
- **L46IG69Y:** The measurement was of the WRONG POPULATION — box un-ticked (7267a72)
- Record the index.lock lesson in wikimem; fix two self-contradictions (071b479)
- **8BXMNQ4T:** Step 1 answered for the measurable half; backburner + review-after (258d442)
- **FFXGPZEI:** Record janitor#297 as this card's first real-world consequence (359b026)
- **8BXMNQ4T:** Report both statistics; state what the control does NOT establish (daab39c)
- **8BXMNQ4T:** Derive the episode count; retract two manufactured numbers on #297 (f808a77)
- **8BXMNQ4T,QJ5LP4W2:** Split the remedy out; fix a STATE block that outlived its card (ae4a96d)
- **BH32A1A5,tests:** Check every link in the env chain; correct an UNDER-claim (084e7c6)
- **BH32A1A5,tests,#294:** Drop a false lint justification; priority high -> medium; reply to #294 (e2256a4)
- **tests:** Measure the pyright claim instead of asserting it a second time (a309fa2)
- **BH32A1A5,#294:** Verify v3.4.2 by publish TIME; make #294's defect state machine-visible (54ea794)
- **BH32A1A5:** The release check was paginated at 100 of 239 — recomputed, answer unchanged (eda1508)
- **5EHBPH6G:** Measure the open box — clause 1 passes, clause 2 FAILS, box stays open (02aa1c9)
- **5EHBPH6G:** Attribute all 211 lock deferrals to marketplace-refresh — 211/211, zero outside (86740aa)
- **5EHBPH6G:** Reword clause 2 to the interval condition; drop an unearned session-boundary claim (3435b5a)
- **5EHBPH6G:** The STATE block contradicted itself — mark the superseded half explicitly (d27f6db)
- **5EHBPH6G:** Archive the terminal card and drop its stale review-after (6ff99c5)
- Correct the record on 5EHBPH6G's decisive fact; disclose two rule violations (3f97631)
- **memory:** State the 600s bound in ATOM-PLJ6-NM0P, and retract a false derivation for it (ee3f560)
- **memory:** Delete the 600s provenance claim instead of replacing it a third time (a0ac708)
- **trdd:** Record the clause-12 exception on the card, where it is discoverable (1f5bc55)
- **trdd:** KE88RIKX box 6 needs a positive control — absence alone cannot fail (ca51c1a)
- **trdd:** Separate the LAW's test from its LOG-OBSERVABILITY test, and probe both (31571b7)
- **trdd:** KE88RIKX — the probe shows the MECHANISM, not the OCCURRENCE half (a) measures (c7a34d1)
- **trdd:** 7NSRD8OV names two different deciders — it is not self-unblockable (10769f7)
- **trdd:** Retract a FALSE pre-registration claim on 7NSRD8OV, and un-pool the tally (d873c0e)
- **trdd:** BH32A1A5 resolved in e4dd674d — todo -> testing, all 4 boxes discharged (0140ac9)
- **trdd:** Add TRDD-Q8PNPRTW for 11 suite failures; correct BH32A1A5 per adversarial review (81c9373)
- **trdd:** Q8PNPRTW — serial isolation finished, 7 of 11 are real (not load) (0ff898c)
- **trdd:** BH32A1A5 complete -> blocked; Q8PNPRTW settle the count and un-overclaim the run (90ef4dd)
- **trdd:** BH32A1A5 back to complete — it was blocked by its own hedge, on nothing (d340c49)
- **trdd:** Add TRDD-0PIPQ77A (rescued from a frozen card); soften two overclaims (97ac55a)
- **trdd:** Record the pretextual column round-trip; clear an orphaned index.lock on provenance (357e543)
- **trdd:** Correct the lock-clearing record — argument order, unproven identity, benign exit 2 (509e9ef)
- **trdd:** Q8PNPRTW is NOT done — the triage count is 5 fixed / 1 not (dea96e9)
- **trdd:** Retract "it is NOT fixed" — the fix works, it just does not survive load (efb5907)
- **trdd:** Confirm the failure mechanism from the traceback; demote a miscounted defect (1e46017)
- **trdd:** Retract "one cause, two tests" — the shared symptom discriminates nothing (c54627f)
- **trdd:** Retract the E-line census claim — it was never the whole evidence (TRDD-Q8PNPRTW) (03290b3)
- Add TRDD-3BQM5GH7 — design cards gate the publish despite markdownlintignore (aafa543)
- **trdd:** TRDD-3BQM5GH7 — .mega-linter.yml is INERT, so option 1 was dead on arrival (7092bde)
- **trdd:** TRDD-3BQM5GH7 — narrow the INERT claim to the phase it was proven for (76ee32c)
- **trdd:** TRDD-3BQM5GH7 — the STATE headline overclaimed scope in the other direction (b0c0fd5)
- **trdd:** TRDD-3BQM5GH7 — experiment eliminates one of three selection rules (538c188)
- **trdd:** TRDD-3BQM5GH7 — probe 2 proves an UNTRACKED file gates the release (1579972)
- **trdd:** TRDD-3BQM5GH7 — the git stash claim was wrong in two of three cases (a04031e)
- **trdd:** TRDD-3BQM5GH7 — scope the probe conclusions to what was actually probed (def13c3)
- **trdd:** TRDD-3BQM5GH7 — probe 3 determines the mechanism: markdownlint globs the filesystem (cf41607)
- **trdd:** TRDD-3BQM5GH7 — walk back three overclaims from the probe-3 write-up (e577bfc)
- **trdd:** TRDD-3BQM5GH7 — retract the "free elimination"; it eliminated nothing (8cc2fc7)
- **trdd:** TRDD-3BQM5GH7 — the probes measured REPORTING, so stop saying selection (e86bed6)
- **trdd:** TRDD-3BQM5GH7 — final precision pass; the review verdict is clean (0c54b25)
- **trdd:** TRDD-3BQM5GH7 — the surviving family is already pruned inside two branches (52829e6)
- **trdd:** TRDD-3BQM5GH7 — fix which observation kills each half-pruned branch (ff9ba9d)
- **trdd:** TRDD-3BQM5GH7 — the lesson is not "oldest evidence wins" (4ce9b73)
- **trdd:** TRDD-3BQM5GH7 — temp-copy died at PROBE 1, on content not presence (4f6bcea)
- **trdd:** TRDD-3BQM5GH7 — "which probe killed it" was the wrong question (eae7cf5)
- **trdd:** TRDD-3BQM5GH7 — the refutations are many-to-many, and the rows are not a partition (989df58)
- **trdd:** TRDD-3BQM5GH7 — sparse-checkout was a self-contradiction and a category error (4e3b893)
- **trdd:** TRDD-Q8PNPRTW — the Captured sections do not exist; root cause is a missing entry marker (383afc6)
- Add TRDD-K7WQ2NRB — split rows 1 & 2 out of Q8PNPRTW; fix two frontmatter fields (9360718)
- **trdd:** TRDD-Q8PNPRTW — add the missing Approval log; correct why min-approval-requirement is user (67bc5b9)
- **trdd:** TRDD-K7WQ2NRB — fd headroom measured (~1M); the bug is load-gated and the box went quiet (b8e6c76)
- **trdd:** TRDD-Q8PNPRTW — state the USER decision in one line; preserve the evidence off /tmp (59e2ce6)
- **trdd:** TRDD-Q8PNPRTW — skim guard so a retracted blocked-by is not read as current (c14730b)
- **trdd:** TRDD-3BQM5GH7 — my own commit message hedged less than the card (76a9bd5)
- **trdd:** TRDD-3BQM5GH7 — 37 needs an upper bound, and I had deleted its source (11d67ed)
- **trdd:** TRDD-3BQM5GH7 — it was 45, measured; I argued twice for the wrong number (625331d)
- **trdd:** TRDD-3BQM5GH7 — 45 confirmed under the repo config; pre/post-fix coordinates reconciled (e82b28e)
- **trdd:** TRDD-QJ5LP4W2 — foreground dispatch IS registration order, and the fix already exists one lane over (738e5f6)
- **trdd:** TRDD-74AA4PAL — compacted sessions are neither woken nor told a handoff exists (3dca8d3)
- **trdd:** TRDD-OES0NN3F split from TRDD-74AA4PAL — and every count in 74AA4PAL was wrong (0435b26)
- TRDD-2640RYR5 — the suite is green; close the card (6b97fbe)
- TRDD-K7WQ2NRB — two in-config full-suite greens, and what they do not license (ce35de3)
- TRDD-K7WQ2NRB — right-size last commit's entry, and close the skip gap it left open (8c0a30a)
- TRDD-K7WQ2NRB — the skip question is moot by arithmetic; date the step-0 insertion (e5acb97)
- TRDD-K7WQ2NRB — the step's own cost estimate was wrong, and that is the first result (47a338b)
- TRDD-K7WQ2NRB — I blamed load for the 10x and the artifacts say otherwise; retracted (191794f)
- TRDD-K7WQ2NRB — "ambient load" was the wrong quantity, and the census fix was worse than the bug (eefa00b)
- TRDD-K7WQ2NRB — the survivor census was blind twice; the third version has a positive control (a6c20e8)
- **memory:** Land the repair chore's 5 PROJECT-scope fixes, which were never actually committed (b004119)
- **memory:** Correct b004119e — its verification was a subset asserted of a set (2b41d19)
- TRDD-K7WQ2NRB — the STATE block was a day stale, and the harness churn is now the failure mode (ba0f98e)
- TRDD-K7WQ2NRB — todo -> backburner; the column was inviting the churn the STATE block forbids (0b9b308)
- TRDD-K7WQ2NRB — state the case against the backburner move on the card itself (d07f32d)
- TRDD-K7WQ2NRB — the STATE block had inverted into a record of my process; trim it to what a resumer needs (602d507)
- TRDD-1PDCPIZC — box 3 observed live; close the card (testing -> complete) (62858e3)
- TRDD-1PDCPIZC — correct a false claim in 62858e33 and record the board verification (e3263c3)
- TRDD-ZQ02QG1L — the blocker completed two days ago and nothing noticed; close the card (87e1300)
- TRDD-ZQ02QG1L — the attention clause did NOT find this card; I did, and 87e13001 said otherwise (7caa53e)
- TRDD-AM8JD9SG — document the unblock-when field on the card, not only in a commit message (a401838)
- TRDD-OES0NN3F — the open box is NOT satisfied by this session's clear; record the check so nobody ticks it (795ad79)
- TRDD-34GB6XUI — closing ZQ02QG1L falsified this card's box 3 and made its task forbidden (8df9e38)
- TRDD-34GB6XUI — my option 2 contained the violation it existed to avoid (eb2b69a)
- **memory:** Split janitor-publish-pipeline hub into 4 sub-pages (SPLIT chore) (b7f08cf)
- **memory:** Closing a card changes its dependents' permissions — and the scope move ate the page once (3a5f5a5)
- **memory:** Un-truncate the law atom's triage line (d714295)
- **memory:** Record the scope-move hazard where it will be READ, and restore the clause I dropped (069c3e1)
- **memory:** Separate the measured outcome from the inferred cause in the scope-move gotcha (36c7093)
- **memory:** Replace the severity-inferred precedence claim with the code-backed one (9447144)
- TRDD-K7WQ2NRB — the STATE block was understating its own evidence (2b017d7)
- TRDD-K7WQ2NRB — withdraw the census "negative", which tested nothing (5802af5)
- TRDD-K7WQ2NRB — the withdrawal over-corrected, and the rate bound needed computing (b1f5bd9)
- TRDD-K7WQ2NRB — two backwards claims, corrected; and this card is now DONE being edited (f8a1fa7)
- **memory:** Persist the review-fork criterion, which existed only in one transcript (afc3746)
- **memory:** The review-fork rule was unsound — it suppressed review of my own worst errors (6e65c88)
- **memory:** State the review-fork rule's real breadth, and guard it against being a rationalisation (d033d70)
- **memory:** Strike an uncounted number from the page that warns against uncounted numbers (ee5f440)
- TRDD-8BXMNQ4T — the card's blocking premise was false, and a rotated log hid it (78d0597)
- TRDD-8BXMNQ4T — todo -> backburner; the column was claiming pullable (f144170)
- TRDD-QJ5LP4W2 — add a STATE block; the advisor gate cannot be met in this session (1d5aab5)
- TRDD-8BXMNQ4T + QJ5LP4W2 — four claims from the last commits over-reached; corrected (a7aab9b)
- TRDD-OES0NN3F — ran the card's own recipe; it would have mis-ticked its last box (f1601b5)
- A third review — I fixed a truthfulness finding with an invented timestamp (d0500dd)
- TRDD-3T9HQEQ6 — dating the tag I had only inferred found a second false claim (3970dd4)
- **memory:** What a 4-fork review chain taught about reviewing corrections (bcfecbc)
- TRDD-3T9HQEQ6 — proved the tag-membership claim properly, and killed an invented count (a441930)
- TRDD-3T9HQEQ6 — I proved tag membership when the claim was about the INSTALLED tree (6694ba5)
- **memory:** 3-of-3 was 4-of-4, the sequence dropped the fork that corrected it (f370b46)
- TRDD-3T9HQEQ6 — closed the one-file gap in the artifact proof by checking, not assuming (a685e7b)
- **memory:** The enumerate-pass has a blind spot — it does not audit its own enumeration (0b973b7)
- **memory:** One criterion, stated once — drop the duplicated stop clause (e9bef6b)
- TRDD-IEAZQ9MK — the "fleet sweep command" its box points at does not exist (b49e7a7)
- TRDD-IEAZQ9MK — sweep box SATISFIED; ran the detectors myself rather than trust the report (8a76385)
- TRDD-IEAZQ9MK — the tick stands, but two of its three evidences did not (821d703)
- TRDD-IEAZQ9MK — its "UNRELEASED, cannot check yet" park expired a day ago (d65bcbc)
- TRDD-IEAZQ9MK — fill implementation-commits, which was empty on shipped code (3a74eda)
- TRDD-IEAZQ9MK — "third card" was a count without members; it is the second (a82ea00)
- TRDD-IEAZQ9MK — add the test commit to implementation-commits (b26d31d)
- TRDD-IEAZQ9MK — I certified my own work; retracted, and routed to the human (31707f5)
- The atom about not minting resolutions minted one — "defensible" removed (f9d336d)
- **memory:** "both are irregular" was still a ruling — three words removed (e1091c4)
- Answer QJ5LP4W2's 78 s question from source, and split out the read it found (TRDD-QJ5LP4W2, TRDD-ZVZAFQY6) (4539b67)
- Demote the 78 s attribution from answer to unconfirmed candidate, and measure instead (TRDD-QJ5LP4W2, TRDD-8BXMNQ4T, TRDD-ZVZAFQY6) (6cd8583)
- Keep the STATE no stronger than its source, and name the shape the numbers actually allow (TRDD-QJ5LP4W2, TRDD-8BXMNQ4T) (1e88f00)
- Box 3 still specified a harness against the retracted premise; plus a candidate checked and killed by its base rate (TRDD-QJ5LP4W2) (6f08e08)
- Actually test the memory-pressure hypothesis instead of computing the odds of the streak I noticed (TRDD-QJ5LP4W2) (3e81051)
- Parameterize the harness target in the STATE bullet too, not only in box 3 (TRDD-QJ5LP4W2) (4a56dc1)
- Verify the attribution the null depended on, and bound what n=6 can say (TRDD-QJ5LP4W2) (8dfda91)
- Confirm the attribution from the CODE, not just from the log it was derived from (TRDD-QJ5LP4W2) (31956db)
- 3BQM5GH7 sat in `blocked` with no blocked-by at all (TRDD-3BQM5GH7) (882a691)
- Registration order only orders tasks due in the SAME pass — so check the intervals too (TRDD-QJ5LP4W2) (f8934d2)
- Name which arm the one ambiguous row falls in — it is not the fragile one (TRDD-QJ5LP4W2) (de36356)
- Compress QJ5LP4W2 — six rounds of self-correction had made it harder to use than it was wrong (TRDD-QJ5LP4W2) (4c3ae8e)
- Restore the three items the compression dropped (TRDD-QJ5LP4W2) (d2631df)
- Drop four circular words from the in-body-guard tally (TRDD-QJ5LP4W2) (14c35a8)
- ZVZAFQY6's own proposed fix is UNSAFE — refused, card re-scoped to the comment (TRDD-ZVZAFQY6) (98ad5c2)
- State what the safety argument actually proves — dedup WIDENS an irreducible race (TRDD-ZVZAFQY6) (bd44af2)
- 1QJIZFFW box 5 cannot tick — nothing invokes --phase after; testing -> dev (TRDD-1QJIZFFW) (67c4f8b)
- My "nothing invokes --phase after" was a negative claim from ONE grep, and it was wrong (TRDD-1QJIZFFW) (3aa2608)
- **clear-lane:** Scope the CLAUDE_PLUGIN_ROOT claim, and record how to tell a skipped run from a failed one (TRDD-1QJIZFFW) (e9dca3d)
- **daemon:** Put the refusal at the code, where the next reader meets the three reads (TRDD-ZVZAFQY6) (60a999e)
- ZVZAFQY6 -> human_review; I will not close a card on my own verdict (TRDD-ZVZAFQY6) (34227c8)
- Cite BOTH halves of the state= claim, bump the column change, and file the board observation (TRDD-ZVZAFQY6, TRDD-5OR85VHP) (3262855)
- Make 5OR85VHP's count reproducible instead of eyeballed (TRDD-5OR85VHP) (d8c1770)
- 5OR85VHP argued a FAKE constraint against one of its own options (TRDD-5OR85VHP, TRDD-ZVZAFQY6) (ea91fde)
- Put the options before the apology — 5OR85VHP is fit to hand over now (TRDD-5OR85VHP) (802bb4f)
- Archive 34GB6XUI (cancelled), ZVZAFQY6 (complete), IEAZQ9MK (complete) — autonomous-drain rulings (TRDD-34GB6XUI, TRDD-ZVZAFQY6, TRDD-IEAZQ9MK) (c59e84c)
- Five blocked-on-decision cards ruled — 56d24c02/WP7TCRME/3BQM5GH7 complete, 74AA4PAL approved, 7NSRD8OV back to testing (TRDD-56d24c02, TRDD-WP7TCRME, TRDD-3BQM5GH7, TRDD-74AA4PAL, TRDD-7NSRD8OV) (2c0a056)
- Review corrections on eight rulings; 5OR85VHP complete via PRRD S12.1; V07NFXS9 filed to retire the dead rungs (TRDD-5OR85VHP, TRDD-56d24c02, TRDD-V07NFXS9, TRDD-QJ5LP4W2, TRDD-STLUO7MB) (d027b70)
- YWMKNKVT complete (blocker already resolved on disk); Q8PNPRTW human_review -> dev, characterisation refused (TRDD-YWMKNKVT, TRDD-Q8PNPRTW) (c503ea5)
- S12.1 add reverted to a PROPOSAL; V07NFXS9 guard instruction made safe; 9ZPU69UC/A70YJLXN ruled on the hub session's answers (TRDD-5OR85VHP, TRDD-V07NFXS9, TRDD-9ZPU69UC, TRDD-A70YJLXN, TRDD-QJ5LP4W2) (258191e)
- The 10:32 full-suite run was RED, not green — 38 failures, exit 3; both suite cards re-grounded (TRDD-7NSRD8OV, TRDD-Q8PNPRTW) (623f48c)
- All 38 suite failures pass alone (serial and -n auto batch); the mechanism experiment is specified, not launched on a load-22 host (TRDD-Q8PNPRTW) (0896838)
- A load-gated runner is armed for the -n auto vs -n 4 experiment; results land in reports/suite-soak (TRDD-Q8PNPRTW) (f98b0cc)
- 74AA4PAL — the orphaned-pusher residual is bounded by the next heartbeat, not by the flag's lifetime; resume_trigger's check read first-hand (TRDD-74AA4PAL) (6522dc8)
- Q8PNPRTW — the runner's two caveats (start-only load gate, per-pass deadline) and what the 38-batch does NOT discriminate (TRDD-Q8PNPRTW) (2e7b2ce)
- A70YJLXN — the flag raiser is the janitor's own detector (version-update.py:115, sole writer); STATE block + rider added (TRDD-A70YJLXN) (8cfbf77)
- Add TRDD-3VIXO8FA — the keychain denied-latch treats a load-induced security timeout as a denial and blinded rotation 14:30-15:09 (TRDD-3VIXO8FA) (3e99c24)
- 3VIXO8FA — 'load' withdrawn as the cause (a correlation); security stalled machine-wide per two instruments, cause not established; denial markers keep latching on every op; peer RA2ZSTOF shared-contract requirement recorded (TRDD-3VIXO8FA) (2c26db5)
- 3VIXO8FA + wikimem page — 'machine-wide' narrowed to two independent process trees (ps ancestry), slope arithmetic corrected (0.4-3.6 %/min, trigger 1-7.5 min before the wall), fix scope stated (transient stalls only) (TRDD-3VIXO8FA) (a99d68a)
- **wikimem:** Rotator latch page — atom rewritten via memgrep (two process trees, noisy slopes, transient-stall scope). The a99d68a6 subject named this page change, but that commit carried only the card: the Edit-tool write was refused by the memory guard, which is the correct path (TRDD-3VIXO8FA) (6d3bd77)
- 3VIXO8FA + wikimem — 'the latch is right' narrowed to anti-flood-correct-but-blind; two processes same securityd, not two keychain contexts; -w cascade bounded by a new acceptance box; lesson on load-adjacency (TRDD-3VIXO8FA) (baccc18)
- 3VIXO8FA — the -w cascade box re-scoped: the daemon path has no -w read (headless gate), only the session-context beacon refresh, one per call; 'anti-flood correct' → 'does what it was built for, cause unmeasured'; 'coincidence' → 'untested correlation'; 'same securityd' → not checked (the baccc183 subject's 'anti-flood-correct' was itself a rebrand) (TRDD-3VIXO8FA) (b37337d)
- **wikimem:** Rotator latch atom — 'anti-flood correct' → 'does what it was built for, cause unmeasured'; 'coincidence' → 'untested correlation'; 'same securityd' → not checked. Lesson ATOM-0FV1-QMUK still prescribes 'coincidence' and is a process lesson; its relocation to the causal-claims methodology page is queued on the card (TRDD-3VIXO8FA) (c446194)
- 3VIXO8FA — cascade box pinned to the run_security seam (the secret-tool fallback is a plain spawn) and to the stall case only (the success path's slot loop issues -w reads per slot) (TRDD-3VIXO8FA) (b9fedb0)
- **wikimem:** Rotator latch page — bidirectional link to the causal-claims methodology page, where the corrected load-adjacency lesson now lives (ATOM-TCGB-UM4V, USER scope); the page-A lesson ATOM-0FV1-QMUK stays (no verb removes a standalone lesson) and is superseded by the link (TRDD-3VIXO8FA) (e52e9de)
- Add TRDD-Q3WSQ9M5, TRDD-KI0H9C8N, TRDD-LDSCQ0NU — backburner cards for the three open issues no card cited (janitor#298 corpus_digest field subset, #299 claudemd_slim topic grouping, #300 memory-split empty-claim-pool spawn); authored by a lean-worker from the issues' own text, verified by the coordinator (frontmatter, external-refs, no handles) (f48035a)
- Archive TRDD-3VIXO8FA and TRDD-V07NFXS9 → complete (2be78271, d9b74bb0) (1bfc3e8)
- **claude-md:** Wikimem index digest regenerated by the post-compact hook (67 pages) (b62b486)
- A70YJLXN — the peer's option-4 commit is live on this host but not on GitHub (TRDD-A70YJLXN) (455740b)
- A70YJLXN — bound the option-4 claim to what was measured; drop one box's runtime state from the pushed card (TRDD-A70YJLXN) (f83afbb)
- A70YJLXN — the 'diff is empty' check was vacuous (zsh passed three paths as one); re-measured (TRDD-A70YJLXN) (ab56f11)
- Add TRDD-9FONCK33 (replay harness, EHT of QJ5LP4W2); QJ5LP4W2 implementation-commits + next action (TRDD-QJ5LP4W2) (4aa50d5)
- 9FONCK33 backburner -> todo — an EHT parked in backburner would park its parent (TRDD-9FONCK33) (ebff733)
- QJ5LP4W2 implementation-commits += 57a7f267 (TRDD-QJ5LP4W2) (2a6921f)
- Implementation-commits on A70YJLXN (3903ca44, ab381b19) and 9FONCK33 (f2f7656d) (497b253)
- 9FONCK33 updated bump for the implementation-commits change (TRDD-9FONCK33) (7f1a37c)
- LDSCQ0NU implementation-commits [9681eb7c]; KI0H9C8N STATE records that the topic switch is corpus-global (TRDD-LDSCQ0NU, TRDD-KI0H9C8N) (fde012e)
- Add TRDD-JDIJ76SW — _TRDD_ID_RE drops v1-migrated bare TRDD-<8hex>-<slug>.md cards (TRDD-JDIJ76SW) (3409ef5)
- Withdraw LDSCQ0NU's "no directory mismatch" verdict; add EHT TRDD-N1CPV1QV (claim step gets the absolute state dir) and TRDD-IB5B14QQ (orphan detector blind to the per-dispatch pool) (TRDD-LDSCQ0NU, TRDD-N1CPV1QV, TRDD-IB5B14QQ, TRDD-JDIJ76SW) (ed8906c)
- N1CPV1QV redesigned after review (state dir travels in the spawn prompt, never on stdout); add TRDD-Q7X4M2KP (scheduler supersedes its own unclaimed record); JDIJ76SW refs no longer cite non-existent TRDD ids (TRDD-N1CPV1QV, TRDD-Q7X4M2KP, TRDD-IB5B14QQ, TRDD-JDIJ76SW) (51b0f51)
- LDSCQ0NU implementation-commits += 61dcfdea (the findings-ledger/docstring/relay-scope follow-up) (TRDD-LDSCQ0NU) (127b661)
- N1CPV1QV fails loudly on a missing or empty state dir; Q7X4M2KP names the in-flight TTL lapse as the duplicate mechanism and supersedes by rename (TRDD-N1CPV1QV, TRDD-Q7X4M2KP) (3c25cbb)
- JDIJ76SW implementation-commits [5ea63752] (TRDD-JDIJ76SW) (d654632)
- N1CPV1QV requirement (e) — the state dir travels inside the dispatch record and the claim refuses on a resolved-path mismatch; Q7X4M2KP superseded prefix joins the prune cap; JDIJ76SW records the digit-uid trade-off (TRDD-N1CPV1QV, TRDD-Q7X4M2KP, TRDD-JDIJ76SW) (4b7ea7e)
- JDIJ76SW targeted suite box ticked — 24 files naming the matcher or its callers, 654 passed; the unscoped run stays with the publish gate (TRDD-JDIJ76SW) (d174336)
- JDIJ76SW suite box back to open — the targeted evidence sits beside the unscoped criterion instead of replacing it (TRDD-JDIJ76SW) (7fe9d99)
- Add TRDD-TASA9ACJ — conftest _source_manifest walks+sorts the 101k-file memgrep/target tree before filtering it; 7NSRD8OV records the unit-8 pair (nauto 17F, n4 20F, 13 common) as RED and lists TASA9ACJ as EHT (TRDD-TASA9ACJ, TRDD-7NSRD8OV) (eec248f)
- 7NSRD8OV category-D column read from traceback sections (1 and 2 raised, not grep -c 7 and 10); Q8PNPRTW gets the 13-id both-runs core; TASA9ACJ leads with the 200x entry ratio, seconds marked load-contaminated (TRDD-7NSRD8OV, TRDD-Q8PNPRTW, TRDD-TASA9ACJ) (a8b4fb9)
- TASA9ACJ implementation-commits [0f2edc79], line-number citations replaced by names; Q8PNPRTW dev -> blocked on the USER waiver, 13-id core is 10 known + 3 new (comm + grep, not a name-read); 7NSRD8OV updated bumped for the category-D correction (TRDD-TASA9ACJ, TRDD-Q8PNPRTW, TRDD-7NSRD8OV) (67aa599)
- Q8PNPRTW blocked-by uses the board's condition-slug grammar in the frontmatter too (TRDD-Q8PNPRTW) (0c2daf6)
- Add TRDD-KS41G6AL — test_inject_still_wanted StopIteration since HMLS5WE8 phase 2 re-reads the pane in the verified wait; fails in isolation, not a suite-load effect (TRDD-KS41G6AL, TRDD-HMLS5WE8) (94ec625)
- Q8PNPRTW names KS41G6AL as owner of the inject_still_wanted pair and records the empty-stdout cluster as a hypothesis with its refuter; HMLS5WE8 gets an append-only pointer that phase 1 (6803ade0) introduced the reads that broke that test file (TRDD-Q8PNPRTW, TRDD-HMLS5WE8, TRDD-KS41G6AL) (9f62471)
- KS41G6AL/HMLS5WE8/Q8PNPRTW — the reddening commit is 6803ade0, not 0c7037bc; the earlier bisect measured the parent twice through a snapshot-dir name collision (TRDD-KS41G6AL, TRDD-HMLS5WE8, TRDD-Q8PNPRTW) (dff5ccf)
- Add TRDD-9EAQS97B — run_subprocess swallows timeouts into stdout='' rc=0; Q8PNPRTW's stamp-leak hypothesis refuted by a first-fire failure; 7NSRD8OV's category-D count is a floor (TRDD-9EAQS97B, TRDD-Q8PNPRTW, TRDD-7NSRD8OV, TRDD-HMLS5WE8) (44deda7)
- **IB5B14QQ:** STATE records the measurement on a copy of the real pool — 14 pending, 1 genuine orphan (enrich LOCAL 47.9h), 13 not-current leftovers correctly silent; candidates() has no filter; seen-file dead-key ceiling noted (TRDD-IB5B14QQ) (641890e)
- **ARTTXA7P, HXZ8B0IS:** EHT card moves to testing with its landing commit recorded; parent records e9dea96a + a02c58de and notes the EHT landed (TRDD-ARTTXA7P, TRDD-HXZ8B0IS) (c8b02d6)
- Add TRDD-PJD6XV66 — bpl's direct gh calls fail soft with no trace; 7NSRD8OV STATE records the guard family's measured cause; two more recall keywords + desc-form note (TRDD-PJD6XV66, TRDD-7NSRD8OV) (4993d56)
- **7NSRD8OV, PJD6XV66:** The guard family's child ceiling was 100 s, so timeout is the least likely of FIVE swallowed reasons; all 7 stamps verified by full name (TRDD-7NSRD8OV, TRDD-PJD6XV66) (9ca18ef)
- **7NSRD8OV, PJD6XV66:** The EAGAIN mechanism is marked HYPOTHESIS, candidates unranked; the 100 s ceiling's three preconditions recorded as verified (TRDD-7NSRD8OV, TRDD-PJD6XV66) (468c9bf)
- Add TRDD-S2RZHXU7 — /janitor-rotate-account-to skill + rotate_to.py, headroom-driven target selection when no account is named (TRDD-S2RZHXU7) (c64ec1c)
- **S2RZHXU7:** Selection rule restated in the knobs' own terms — both are percent-USED thresholds, so candidate ⇔ max(5h,7d) <= SCOPED_ACCOUNT_HEADROOM and Fable headroom ⇔ util < SCOPED_SWITCH_AT (TRDD-S2RZHXU7) (ba3b559)
- **S2RZHXU7:** The verb is NEVER gated on chore ownership or the harness — USER rationale recorded verbatim; per-slot Fable named by its real label 7d/Fable + util_pct; absent-window rule settled (TRDD-S2RZHXU7) (5bdd0f3)
- **S2RZHXU7:** An empty usage probe is UNKNOWN, never "0 % used"; an expired-token slot is never auto-picked (TRDD-S2RZHXU7) (c7a19fb)
- S2RZHXU7 → testing (landed ae96dd12); add TRDD-M4HVFU2A — model-fallback keeps typing /model opus after a rotation onto an account whose Fable window is not enforced (TRDD-S2RZHXU7, TRDD-M4HVFU2A) (1a87a80)
- CLAUDE.md wikimem index regenerated (janitor claudemd-migration-queue drained on compaction) (229cdcf)
- PJD6XV66 → testing (eb4bce28); M4HVFU2A → testing (772e46a1) — is_active measured true at 97 %, bar is 100 % only, backoff per project, 90→100 named as a policy change, default ON kept, interim cooldown stamp NOT done (TRDD-PJD6XV66, TRDD-M4HVFU2A) (845f5e0)
- PJD6XV66 — isolated green proves only "not deterministic", 5 of 7 gate failures unnamed, FIRE asserts carry stderr; M4HVFU2A — shared headroom predicate + no-arg verb (2d8cf86e), rotate-first is advice-only, peer's server-bar root cause + 90-vs-95 follow-up recorded (TRDD-PJD6XV66, TRDD-M4HVFU2A) (d5b39f2)
- **rules:** Compress the janitor#260 clause in the heartbeat memory row — shipped rules were 491 B over the 53 700 B floor cap and blocked the publish; PJD6XV66 + M4HVFU2A STATE corrections from review (TRDD-PJD6XV66, TRDD-M4HVFU2A) (ff4814a)
- **rules:** Restore the general "never decline on your own measurement" prohibition in the heartbeat memory row; PJD6XV66 names the 39-min gate's two captured failures from its report; M4HVFU2A qualifies the wall claim (TRDD-PJD6XV66, TRDD-M4HVFU2A) (222152a)
- **rules:** The enrich exception states its reason again ("its defects ARE lint rules") so it cannot read as contradicted by the next clause; provenance-only trims pay for it (TRDD-PJD6XV66, TRDD-M4HVFU2A) (902d0ae)
- **rules:** The abstain clause sits next to its antecedent (the skill's candidate step) so "its" no longer reads as enrich's; "(so it re-fires)" restores the reason to leave the pending JSON (TRDD-M4HVFU2A) (2d420b7)
- **memory-skills:** Move claim exit-code detail, STATE_DIR rationale and review anecdotes from four SKILL.md bodies into their references/ — each body back under the 5000-token cap that blocked the publish (TRDD-N1CPV1QV) (1a4d8ff)
- **LDSCQ0NU:** Redact an absolute home path in the card body — CPV --strict CRITICAL "private path leaked" blocked the 3.4.15 publish at gate [4/11] (TRDD-LDSCQ0NU) (3c8b8dc)
- **LDSCQ0NU:** The redacted path carries a dated redaction note instead of a second "on this host" (review of 3c8b8dc8) (TRDD-LDSCQ0NU) (acec526)
- Add TRDD-7MGJYLY5 — owner ruling on compaction: prefer /clear + llm-ext at turn boundaries, no janitor handoff on autocompact, preserve subagents, restore skills not files (a73f073)
- **7MGJYLY5:** Separate the owner's words from the relaying session's derivation; make acceptance boxes 2/3/5/6 measurable (review of a73f0733) (TRDD-7MGJYLY5) (d1b0aea)
- **7MGJYLY5:** Box 2 no longer forces keying on the compaction window that requirement 1 leaves open; drop this session's unattributed inference from the evidence paragraph (review of d1b0aea4) (TRDD-7MGJYLY5) (9144ff5)
- **7MGJYLY5:** Box 6 no longer asserts the janitor lacks a skill-recording mechanism — nobody verified that; the card now says only that it does not identify one (review of 9144ff5a) (TRDD-7MGJYLY5) (aa3bb5e)
- **memory-skills:** Restore the full per-reference TOC embeds in three SKILL.md Resources lists; add the TOC repair-background.md lacked and the abstain line merge-protocol.md's TOC missed (TRDD-N1CPV1QV) (9af1a1c)
- **design:** Markdownlint repairs that blocked the 3.4.15 publish at gate [4/11] — two tables in K7WQ2NRB ran straight into prose (56 MD056 rows), two blank lines split blockquotes in AM8JD9SG and 74AA4PAL (MD028) (456af64)

### Features

- **handoff:** Convert the second skill to the out-of-process composer (TRDD-ZQ02QG1L) (60f6ad9)
- **8BXMNQ4T:** Add the control that makes the coverage metric mean something (4160789)
- **5EHBPH6G:** CLOSE the card — column complete; record the unsatisfiable-criterion lesson (e4aead3)
- **daemon:** Per-beat foreground budget with a survival floor and oldest-first dispatch (TRDD-QJ5LP4W2) (5f7f3db)
- **version-update:** Say out loud when a self-update rides the 4 h cadence floor (TRDD-A70YJLXN) (3903ca4)
- **daemon:** State the 4 h floor mechanism when version-update is server-owned and no session raised the flag in one absorbed beat (TRDD-A70YJLXN) (ab381b1)
- **claudemd-slim:** Group the wikimem index by metadata.topic when pages carry one; one digest function for header and staleness (TRDD-KI0H9C8N) (f948226)
- **daemon:** Chore-coordination yield/resume lines carry the liveness probe's ts, age and reason; server_liveness_probe() splits absent/malformed/stale/read-error behind an unchanged server_capabilities() (TRDD-HXZ8B0IS) (e9dea96)
- **rotator:** /janitor-rotate-account-to — one-call account rotation via rotate_to.py; no-argument path ranks non-live slots by 5h/7d then Fable headroom, types /model opus first only when every candidate's Fable window is spent (TRDD-S2RZHXU7) (ae96dd1)

### Miscellaneous Tasks

- Bump version to 3.4.14 (1a8627c)
- Bump version to 3.4.14 (4326519)
...
- **triggers:** Delete the five self-triggers' dead blind-osascript fallbacks; re-ask the type-time guard during the verified wait (TRDD-HMLS5WE8 phase 2) (87622b4)
- **fleet-restart:** Retire the unreachable force_restart and resurrect rungs and the written-record ladder (TRDD-V07NFXS9) (d9b74bb)

### Testing

- **publish:** Pin that the gate's checker set is not a subset of CI's (c239822)
- **branch-protection:** Pin the skill's convergence fix by EXECUTING the shipped snippet (71e1e3d)
- **daemon:** Pin the pane= field box 6 greps for, and make the box a runnable check (ab5d352)
- **daemon:** Strengthen the unread assertion — the `or` made it unable to fail (c9bd981)
- **hooks:** TRDD-OES0NN3F — permanent coverage for the compact injection, with the positive control my throwaway arms lacked (57f015b)
- **hooks:** TRDD-OES0NN3F — two of my new tests passed for the wrong reason (e6cc203)
- **hooks:** TRDD-OES0NN3F — cut dead ceremony, and verify the failure mode I documented (9f60125)
- **hooks:** TRDD-OES0NN3F — the defang could have been deleted without reddening the suite (10fef00)
- **hooks:** TRDD-OES0NN3F — the "dead ceremony" I deleted was the only thing catching a gate regression (38e9df3)
- **hooks:** TRDD-OES0NN3F — assert the flag survives BEFORE the once-only calls, not after (6e75aef)
- **hooks:** TRDD-OES0NN3F — the empty-body test now tells silence from a crash (0f00fd6)
- **compose:** Pin the composer's marker prepend, and close TRDD-L46IG69Y per USER decision (01ff426)
- **capture:** TRDD-K7WQ2NRB — row 2 stopped throwing away the only witness to its failure (6268dbe)
- Assert the subprocess return code, matching the helper's own guard (TRDD-IEAZQ9MK) (17aa502)
- **terminal_trigger:** Pin that send_verified forwards still_wanted; card records the column shortcut honestly (TRDD-HMLS5WE8) (6d5463d)
- **daemon:** Replay harness for the worst measured foreground occupancy pattern (TRDD-9FONCK33) (f2f7656)
- Roster guard follows the hub's part links; branch-protection FIRE asserts carry stderr (TRDD-PJD6XV66) (20e63eb)
- Drop the unused _binp binding pyright flagged in test_dedupe_then_rearm (TRDD-PJD6XV66) (6425501)

### Wikimem

- Split janitor-detector-and-hook-roster (52.6 KB) into overview + -list + -findings; backlinks redirected (janitor memory-split chore) (9e115e6)
## [3.4.14] — 2026-09-03
