...
> - **DISARMED** — else if `~/.claude/janitor-control/kill-switch.flag` OR the older
>   `~/.claude/plugins/data/ai-maestro-janitor-ai-maestro-plugins/global-state/kill-switch.flag`
>   EXISTS (set by `/janitor-global-disarm`), the janitor is intentionally stopped → treat this rule as INERT
>   this session.
