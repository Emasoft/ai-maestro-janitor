...
| ~~`~/.claude/projects/<slug>/design/`~~ → `<project-root>/.claude/local/design/` | **LOCAL** TRDD cards moved IN-TREE (TRDD-WY198OIP, ai-maestro#163 / issue #303) to end a split-brain with ai-maestro's own pillar resolver — gitignored by the same `.claude/**` pattern every project already carries. A SessionStart hook migrates any pre-existing `~/.claude/projects/<slug>/design/` once. **Only LOCAL *memory* stays at the slug dir above** — design and memory are different subsystems | n/a — the old path is migrated away, not deleted |
