---
name: janitor-plugin-install
description: Install a Claude Code plugin, adapting automatically to the harness. Use when the user asks to install a plugin, or says "install <plugin>", "install <owner>/<repo>", or points at a plugin URL or local folder. Accepts a bare name, plugin@marketplace, plugin@owner/marketplace, an owner/repo shorthand, an https/ssh URL, or a local directory path.
---

# Janitor plugin install

Install a plugin through whichever backend is correct for THIS session, so the same command
...
Inside the harness the server owns each agent's plugin set, so a direct `claude plugin install`
...
```bash
uv run --script --quiet "${CLAUDE_PLUGIN_ROOT}/scripts/plugin_manage.py" install <target> [--scope user|project|local] [--dry-run]
```
...
ONLY install. Does not arm the heartbeat, edit settings by hand, or touch other plugins.
