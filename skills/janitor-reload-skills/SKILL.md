...
description: Run /reload-skills for the current Claude Code session so freshly installed STANDALONE (non-plugin) skills and commands take effect, without the human typing the command. Use after adding a standalone skill or command at local, project, or user scope — /reload-plugins reloads ONLY plugin-bundled skills, so a skill/command that is not part of any plugin needs /reload-skills instead. Invoke in response to a [janitor-reload-skills] heartbeat marker (emitted after /janitor-global-reload-skills), or whenever a standalone skill/command changed on disk and this session must pick it up. Fires ESC then /reload-skills at this session's own terminal pane (iTerm or tmux). Supports --soft (enqueue without ESC, so the current turn finishes first). Trigger with /janitor-reload-skills, /janitor-reload-skills --soft, or by asking to reload skills now.
---
