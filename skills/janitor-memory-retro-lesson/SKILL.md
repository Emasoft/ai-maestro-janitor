...
description: RETRO-LESSON — the autonomous backfill pass that converts ALREADY-superseded atoms into the lesson form (DO NOT X, BECAUSE why, DO Y instead). Fires on the bare [janitor-memory-retro-lesson] heartbeat marker (or /janitor-memory-retro-lesson). Finds atom markers carrying status:superseded but NO superseded-by: pointer — the not-yet-converted signature — sources each WHY from the commit/TRDD provenance chain (NEVER invents one; unsourceable → FLAG for a human), converts via memgrep add-lesson --supersedes, and completes the atom's superseded-by: pointer through the repair-op transaction. Bounded, crash-safe, disable-able. One of the seven wikimem-editor passes (split/consolidate/conflict/repair/atomize/harvest/retro-lesson).
---
