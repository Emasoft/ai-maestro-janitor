...
`ultracode-workflow.md`. Read that for HOW the fan-out runs; read this for WHAT
each stage does and how the verdict is committed.
...
- **Repo resolution comes from provenance, not a scan.** The repo is the one named
  ...
  repos are plausible → **ambiguous ⇒ DEMOTE**.
...
trace), plus one sentence of evidence."* (Full prompt in the sibling
`ultracode-workflow.md`.)
