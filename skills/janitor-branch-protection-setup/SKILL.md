...
description: Applies the ratified branch-protection baseline to the default branch of this project's GitHub repo — baseline-history-protect, baseline-pr-and-checks and baseline-tag-protect, owner-bypassed, with history rewrite allowed and linear history deliberately excluded. Idempotent — re-running updates in place, never duplicates, and clears an orphaned legacy janitor-baseline ruleset. Trigger with /janitor-branch-protection-setup, "set up branch protection", "harden the default branch", or "add a baseline ruleset to GitHub".
---
...
1. **`baseline-history-protect`** — `bypass_actors: [{actor_id: 5,
   actor_type: RepositoryRole, bypass_mode: always}]` (owner bypass)
   * `deletion` — blocks branch deletion for non-owners
   * **NO `non_fast_forward`** — USER Tier-3 ruling 2026-08-27: "history
     ...
     repos." `non_fast_forward` is GitHub's block-force-push rule, so it
     is removed outright rather than merely bypassed.
   * **NO `required_linear_history`** — deliberately excluded. It forbids
     ...
     merge). `/janitor-github-config-fix` REMOVES this rule from any repo
     still carrying it (an older baseline version added it).
