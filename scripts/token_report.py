...
main agent's default 1-hour cache TTL, ~1.25x at a subagent's default 5-minute one — an
org's `promptCacheTtl`/`subagentPromptCacheTtl` setting can raise either to 1 hour,
flipping that multiplier); `cache_read` is the cheap (~0.1x) context re-read, shown for
context but not the alarm metric.
...
def _price_per_mtok() -> float | None:
    ...
    weighted tokens only (TRDD-ZCODD6YS, Deliverable 1). This is LIST price and knows
    nothing about an org's `modelPricing` managed-setting contracted rates/discount, so
    it can diverge from what `/cost` reports for the same tokens."""
...
def _render_live(as_json: bool) -> int:
    ...
    if usage is not None:
        print(f"  Last turn — output: {usage.output_tokens} (full price)  ·  cache_creation: {usage.cache_creation_input_tokens} (premium write, ~2x at the main agent's default 1h cache TTL — org-set promptCacheTtl can change this, billed once per prefix change)  ·  cache_read: {usage.cache_read_input_tokens} (cheap re-read, ~0.1x)")
    else:
...
def _render_interval(since: int, until: int, as_json: bool, *, graph: bool = False, label: str = "EXACT interval") -> int:
    ...
    print(f"  fleet by category — output {_fmt_k(total['output'])} (full price)  ·  input {_fmt_k(total['input'])} (uncached, full price)  ·  cache_write {_fmt_k(total['cache_creation'])} (premium, ~2x main / ~1.25x subagent at default TTLs — org-set promptCacheTtl/subagentPromptCacheTtl can raise either, once per prefix change)  ·  cache_read {_fmt_k(total['cache_read'])} (~0.1x re-read)")
    print()
