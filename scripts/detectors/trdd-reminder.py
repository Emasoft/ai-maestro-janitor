...
def _session_key() -> str:
    ...
    digest = hashlib.sha1(f"{host}@{today}".encode("utf-8"), usedforsecurity=False).hexdigest()
    return digest[:12]
...
def main() -> int:
    ...
    entries_hash = hashlib.sha1(entries_signature.encode("utf-8"), usedforsecurity=False).hexdigest()[:8]
