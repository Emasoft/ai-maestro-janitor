...
Fires a DETACHED, delayed /reload-skills at THIS session's own pane (tmux or
iTerm, via `terminal_trigger.send_self_command`) so the agent can pick up
freshly installed STANDALONE (non-plugin) skills and commands WITHOUT the
human typing the command. Claude Code's `/reload-plugins` only reloads
skills/commands bundled INSIDE a plugin; a standalone skill or command dropped
into `~/.claude/skills`, `.claude/skills`, `~/.claude/commands`, etc. (local /
project / user scope, not part of any plugin) is picked up by `/reload-skills`
instead. The Skill tool refuses built-in slash commands, so — exactly like the
compact and reload triggers — the only working path is to type the command
into this session's own pane.
...
agent ends its turn). It targets ONLY this session's own pane — never other panes —
so concurrent Claude instances are untouched.
