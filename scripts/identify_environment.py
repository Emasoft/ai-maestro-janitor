...
def _gather_plugins(*, online: bool) -> dict:
    ...
    # marketplace_refresh_ts / marketplace_refresh_fails fields removed here:
    # the marketplace-refresh chore was retired 2026-09-17 (it ran `claude
    # plugin marketplace update` across every registered marketplace and
...
def _render(info: dict) -> str:  # noqa: C901 - a flat report builder; branching is inherent
    ...
    if pl:
        ...
        # marketplace-refresh rendering block removed here: the chore was
        # retired 2026-09-17 (fseventsd 27 GB churn), so `jan` no longer
        # carries `marketplace_refresh_ts`/`marketplace_refresh_fails`.
        lines.append(pline)
