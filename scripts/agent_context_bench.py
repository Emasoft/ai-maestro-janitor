...
def coverage_doc(res: dict) -> str:
    """The per-rule MEASURED coverage table, for `COVERAGE.md`.

    Exists because a rule's NAME reads as coverage. `id="authority-override"` looks like
    authority-override is guarded, so nobody audits it — and a rule that is named but dead is
    WORSE than an absent one: an absent rule invites the question, a dead one forecloses it.
    (ARCHITECT, janitor#254; same shape as the memory lesson that an assertion outlives the
    property it describes and then suppresses the suspicion that anyone should look.)

    So the claim and its measurement are published together. A reader sees
    `authority-override — 0/9 FALSIFIED`, not a name that looks like a guard.
    """
    claimed = claimed_rule_ids()
    pc = res.get("per_class", {})
    rows, dead, unmeasured = [], [], sorted(claimed - set(pc))
    for name in sorted(claimed):
        b = pc.get(name)
        if not b or not b["n"]:
            rows.append((name, "—", "UNMEASURED — not evidence of working"))
            continue
        rate = b["intended"] / b["n"]
        if b["intended"] == 0:
            dead.append((name, b["n"]))
            verdict = "**FALSIFIED** — seeded and caught nothing"
        elif rate >= 0.5:
            verdict = "partial"
        else:
            verdict = "weak"
        rows.append((name, f"{b['intended']}/{b['n']}", verdict))

    seeded = sum(n for _, n in dead)
    out = [
    ...
    out += [f"| `{n}` | {r} | {v} |" for n, r, v in rows]
    if dead:
        out += [
            "",
            f"**{len(dead)} claimed rules are FALSIFIED**: seeded with {seeded} blind-authored",
    ...
    if unmeasured:
        out += ["", "**Unmeasured**: " + ", ".join(f"`{u}`" for u in unmeasured) + "."]
    return "\n".join(out) + "\n"
