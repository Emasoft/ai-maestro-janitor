...
def run_check(client_factory: Callable[[], JevClient] | None = None) -> int:
    ...
    if usage is not None and getattr(usage, "input_tokens", 0):  # type: ignore[arg-type]  # mypy misreads the 3-arg getattr overload set here as requiring a bool default
...
def _check_question_types(client: JevClient) -> bool:
    ...
    # `answers[key]` is typed as the `Answer` union (NoulAnswer | ChoiceAnswer | ScoreAnswer) —
    # pyright cannot narrow it from the *question* type each key was asked with, so the
    # `.noul`/`.choice`/`.score` accesses below are cast to what this specific sample request
    # (three DIFFERENT question types, one key each) is known to produce.
    noul = cast(NoulAnswer, answers["is_question"])
    choice = cast(ChoiceAnswer, answers["topic"])
    score = cast(ScoreAnswer, answers["urgency"])
