...
class MalformedFences(ValueError):
    """The CLAUDE.md text contains a broken JANITOR-REPO-MAP fence pair
...
def _fence_span(text: str) -> tuple[int, int] | None:
    ...
    starts = [i for i, ln in enumerate(lines) if ln.strip().startswith(FENCE_START)]
    ends = [i for i, ln in enumerate(lines) if ln.strip() == FENCE_END]
    if not starts and not ends:
        return None
    if len(starts) != 1 or len(ends) != 1 or starts[0] > ends[0]:
        raise MalformedFences(
            f"expected exactly one START before one END fence; "
...
def has_map_block(text: str) -> bool:
    ...
    return _fence_span(text) is not None


def read_fence_header(text: str) -> dict[str, str] | None:
    ...
    rest = start_line[len(FENCE_START) :].strip()
    fields: dict[str, str] = {}
...
def replace_map_block(text: str, new_block: str) -> str:
    """Swap the existing fenced block for `new_block` (the maintainer's
    on-change path). `new_block` is a full render_block(...) string (fences
    ...
    span = _fence_span(text)
    if span is None:
        raise MalformedFences("no JANITOR-REPO-MAP block to replace; use insert_map_block")
    return text[: span[0]] + new_block + text[span[1] :]


def insert_map_block(text: str, new_block: str) -> str:
    ...
    if _fence_span(text) is not None:
        raise MalformedFences("a JANITOR-REPO-MAP block already exists; use replace_map_block")
    if not text.strip():
        return new_block
    return text.rstrip("\n") + "\n\n" + new_block


def remove_map_block(text: str) -> str:
